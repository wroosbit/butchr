...
- ⚠ **File through `atlassian_create_issue` on the Butchr proxy where you have it, and only reach for the official server's `createJiraIssue` when you do not.** They are two doors into the same Jira site and **only one of them assigns the ticket**. The proxy sets the assignee server-side from the daemon's own account and refuses outright when it cannot; the official tool takes an optional `assignee_account_id` that is undefaulted and unwarned, so **a ticket filed through it is born with an empty assignee**. That is not cosmetic: the board reconciler starts an agent only for `assignee = currentUser() AND status IN ("In Progress", "In Review")`, so an unassigned ticket **can never be staffed**, and it reads exactly like one nobody has triaged yet. 104 had accumulated by 2026-08-21 (KAN-577), and **three more were born unassigned in the ninety minutes after the proxy was fixed**, by an agent that had read this rule at activation and filed through the other door anyway. **This bullet lowers the rate and is not the mechanism** — since KAN-597 `boardControl.health.unstaffable` on `butchr_list_agents` reports every open unassigned ticket whichever door it came through, which is the part that does not depend on you remembering. If you do file through the official server, set `assignee_account_id` to the account you are authenticating as, and check the ticket back afterwards.
