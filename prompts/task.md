...
- **A new `verify-` script must carry the header sentence, a verdict-derived exit, and a call site that can reach it — and CI enforces all three.** Every `verify-*.mjs` under `daemon/scripts` or `extension/scripts` needs (a) a `WHAT FAILURE THIS WOULD CATCH:` line in its header comment naming the defect it would have caught, (b) an exit whose value comes from an accumulated verdict — `process.exit(failures ? 1 : 0)`, `if (failures) process.exit(1)`, or a `process.exitCode` set from a check — and (c) at least one call site that can actually make that verdict negative. A literal `process.exit(1)` guarding "daemon/dist is missing" is a setup guard, not a verdict, and does not count. Neither does a `check()` helper nobody calls, or a `let failures = 0` nothing ever increments: both satisfy (b) completely while the script exits 0 forever, which is why (c) exists. The required `verify-script-sweep` check runs `node daemon/scripts/sweep-verify-exit-paths.mjs` on every PR, so a script missing any of the three goes red before review rather than in it. Run it yourself before you push. Passing it is necessary and not sufficient — it proves the script *can* report failure, never that its assertions can be false: a `check(true, …)` is a call site by every measure the sweep applies. That is still the bullet above, and you still have to watch it go red.
