...
| what Butchr **sends** to ask for a resume | `spawnSession` reads `hasRestorableConversation(workDir)` and picks the framing (`daemon/src/herdr.ts`) | **nothing, and it cannot ask.** `resume` is stored on the local session object in `daemon/src/crabcast-runtime.ts` and **is never sent**: `provision()` sends `path`, `priority`, `refusable`, `chargeable`, `preemptable`, `launcher`, `prompt` and `mcpServers`, and no other field. The three gate flags are KAN-492's, and they are capacity terms — none of them can ask for a conversation back, so the claim this row makes is unchanged by them |
