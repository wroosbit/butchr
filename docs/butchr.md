...
The canonical prompt lives in [`prompts/epic.md`](../prompts/epic.md) — see that file rather than an embedded copy here, so the two cannot drift. In outline: the agent decomposes its epic into Stories, staffs and steers the story agents that carry them, and **reviews and approves the pull requests of the tasks it supervises directly** — approving only with green required CI **and** the ticket's live-proof acceptance criteria re-run against the PR head. **It does not merge.** Merge governance changed on **2026-08-08** (human decision, superseding 2026-08-03): **the story agent approves; the task agent merges**. **The approver is read off the board in three branches** (KAN-239, correcting KAN-237 twice): **(1)** the story the task implements, if any — found by a Jira issue **link** to a Story, since `Story` and `Task` are both `hierarchyLevel: 0` and **Jira structurally cannot parent a task to a story**, with `activatedBy` read only as corroboration where it agrees; **(2)** otherwise the **parent epic's agent**, read off the `parent` field, where `activatedBy` is never consulted at all; **(3)** otherwise the ticket is **mis-filed** — its agent says so and does not merge, and appoints no substitute. The first wording said *"supervisor of record — the agent that activated you"*, which resolved to nobody because `activatedBy` is `null` for every board-started agent; the second said *"parent story's agent, otherwise parent epic's"*, which reads the story off a hierarchy that cannot hold one and would have handed every task to its epic. Both are the same mistake — naming a relation without checking the board can express it. Branch 3 is **permanent and does not depend on KAN-212's status**: a filing rule makes an orphan unlikely, never impossible, and a rule that terminates beats one relying on a convention holding. That the population was empty on 2026-08-08 was a fact about somebody's hygiene rather than the system — three tickets were hand-repaired into compliance that day, one of them 26 minutes before an agent queried it and drew a conclusion from the repair. Approval is a **precondition, not an ordering** — a PR is merged only after somebody other than its author has reviewed it, and green CI is not approval. Nothing mechanical enforces it: every agent authenticates as the same account, so GitHub refuses a formal review verdict on our own PRs (the verdict is a PR **comment**) and leaves the merge button open to the author. See KAN-237. It also owns the epic's durable written record, which since **2026-08-05** (story KAN-160) lives in **two** places rather than one. The epic's Jira **description** holds the **north stars** — the invariants a proposal is measured against — plus pointers to everything else, kept short enough to read every session. The **design doc** is a **Confluence page**: the architecture, the decisions and the reasoning behind them, what was rejected and why, and the agent's **operating memory** — what must persist across a deactivation or reset so a restarted agent picks up where the last one left off. For this repository's own epic, KAN-39, that page is **Butchr — design doc**, space `SD`, page id `1605634`: <https://wroosbit.atlassian.net/wiki/spaces/SD/pages/1605634/Butchr+design+doc>. Point-in-time state ("what is staffed, what is blocked") goes in comments, not into either of them; `prompts/epic.md` is the canonical statement of the rule and the test that applies it.
...
**When the network is down, this file is the one that still reads.** The Atlassian MCP was unreachable for about two hours on 2026-08-04 (KAN-157), and during it the epic agent had neither Jira nor Confluence — which is why the code-facing reference stays in the repo rather than moving to Confluence, and why the link above is a pointer rather than a dependency. If that page does not resolve, or Jira is down with it: nothing you need in order to **act** is out of reach, because behaviour is in `prompts/<type>.md` and the mechanism is in this file, both of them on disk in the repo you already have. What is out of reach is the reasoning, the rejected alternatives and the incident history — plus the epic's north stars, if Jira is down too. That is a reason to defer a decision that turns on *why*, and to say plainly in whatever you produce that you could not read the page. It is **not** a reason to reconstruct the page's contents here from memory: a second unauthoritative copy of the design doc is the failure this split exists to avoid.
