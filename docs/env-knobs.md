...
| `BUTCHR_ALLOW_UNPINNED_RUNTIME` | `1` | unset | Lets the daemon serve even though the systemd unit declares a `BUTCHR_AGENT_RUNTIME` this process is not carrying. **Without it that daemon refuses to start and exits 4** (KAN-550) — on 2026-08-20 a client-spawned replacement dropped the pin silently and ran the fleet on a runtime nobody chose. Exact match on `1`; a truthy value does not count, and setting it is still announced on stderr. **You do not need it to run the `verify-` scripts** — since KAN-574 the pin governs only a daemon that would claim the machine's own socket, and a script that spawns one into a throwaway `$HOME` gets a socket of its own. Reach for this knob when you are starting a daemon on the REAL `$HOME` and mean to run it unpinned. |
