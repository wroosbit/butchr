...
import { describeBuildGap, readTemplateAt, resolvePromptSource } from './prompt-source.js';
/**
 ...
 */
export function templateProvenance(repoRoot, templatePath, source = { kind: 'worktree', because: 'no source was resolved' }, now = new Date()) {
    ...
    if (!line) {
        return {
            ...base,
            unavailable: `git recorded no commit touching ${templatePath} in ${repoRoot} ` +
 ...
 */
export function renderProvenanceBlock(p) {
    ...
    if (!p.commit) {
        lines.push(`- **Which commit \`${p.templatePath}\` came from could not be determined** — ` +
            `${p.unavailable}.`, `- **So you cannot check it from here, and must not assume it is current.** ` +
    ...
    lines.push(`- **\`${p.templatePath}\` last changed in \`${shortSha}\`** — *${subject}* (${date}).`, `- **To find out whether that is still the rule**, run these two — they are the` +
        ` whole check:`, '', '  ```bash', `  git -C ${p.repoRoot} fetch origin`, `  git -C ${p.repoRoot} rev-parse ${shortSha}:${p.templatePath} origin/main:${p.templatePath}`, '  ```', '', `  **Two identical shas mean this brief is current** and you need do nothing else. ` +
        ...
        `and follow what \`origin/main\` says rather than what is written above it.`, '', `  **And if it prints \`fatal:\` instead of two shas, it has told you it cannot answer** — ` +
    ...
    if (p.buildGap) {
        lines.push('', `- ⚠ **The brief is current; the running daemon is not.** ${p.buildGap}. ` +
