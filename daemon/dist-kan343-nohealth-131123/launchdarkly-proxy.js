...
export const LD_PROXY_MODES = ['off', 'launchdarkly-read'];
/**
 ...
 */
export const LD_READ_SCOPE_TOOLS = [
 ...
 */
export const LD_WRITE_SCOPE_TOOLS = [
    'create-feature-flag',
    'update-feature-flag',
    'delete-feature-flag',
 ...
 */
export function enabledLdModes(mode) {
    switch (mode) {
        case 'off':
            return [];
...
function pathKey(args, field, example) {
    const raw = typeof args?.[field] === 'string' ? args[field].trim() : '';
    if (!raw)
        return { error: `${field} is required, e.g. "${example}"` };
    if (!LD_KEY.test(raw)) {
        return {
            error: `"${raw.slice(0, 60)}" is not a usable LaunchDarkly ${field}. Expected a key like ` +
...
function variationKey(args) {
    const raw = typeof args?.variationKey === 'string' ? args.variationKey.trim() : '';
    if (!raw) {
        return {
            error: 'variationKey is required. Read the variation keys of an AI Config with ' +
                'launchdarkly_get_ai_config, which lists them.'
        };
    }
    if (!LD_VARIATION_KEY.test(raw)) {
        return {
            error: `"${raw.slice(0, 60)}" is not a usable variation key. LaunchDarkly mints these as ` +
...
function text(name, maxLength, hint) {
    return {
        name,
        validate(raw) {
...
function keyParam(name, example) {
    return {
        name,
        validate(raw) {
            if (raw === undefined || raw === null || raw === '')
            ...
            if (!LD_KEY.test(value)) {
                return {
                    error: `"${value.slice(0, 60)}" is not a usable ${name}. Expected a LaunchDarkly key like ` +
 ...
 */
function boundedInt(name, min, max) {
    return {
        name,
        validate(raw) {
            if (raw === undefined || raw === null || raw === '')
    ...
    return {
        ...
        validate(raw) {
            ...
            if (!Number.isFinite(asked) || asked < 0 || asked > 4_102_444_800_000) {
                return {
                    error: `${name} must be a Unix timestamp in milliseconds, e.g. 1786480000000. ` +
 ...
 */
function buildQuery(params, args) {
    const parts = [];
...
const LOUD = 'A FAILURE HERE IS ALWAYS LOUD: if the daemon\'s credential is expired, revoked, unreachable, ' +
...
const PLAN_GATED = 'NOTE: AI Configs are a plan-gated LaunchDarkly feature. On an account whose plan excludes ' +
 ...
 */
export const LD_PROXY_OPERATIONS = [
    {
        ...
        description: "List the feature flags in a LaunchDarkly project, through the Butchr daemon's own " +
    ...
    },
    {
        ...
        description: "Read one feature flag's full configuration — variations, targeting rules and per-environment " +
    ...
    },
    {
        ...
        description: "Read one flag's lifecycle status in every environment — new, active, inactive or launched, " +
    ...
    },
    {
        ...
        description: "List a project's environments through the Butchr daemon's own credential, including which " +
    ...
    },
    {
        ...
        description: "Read LaunchDarkly's audit log through the Butchr daemon's own credential — who changed " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                spec: {
                    type: 'string',
                    description: 'Optional. A LaunchDarkly resource specifier, e.g. "proj/butchr:env/*:flag/agent-runner", ' +
    ...
    },
    {
        ...
        description: 'List the repositories LaunchDarkly has code references for, optionally narrowed to one ' +
    ...
    },
    {
        ...
        beta: true,
        description: "List a project's AI Configs through the Butchr daemon's own credential. Bounded at " +
    ...
    },
    {
        ...
        beta: true,
        description: "Read one AI Config — its variations and model settings — through the Butchr daemon's own " +
    ...
    },
    {
        ...
        beta: true,
        description: "Read one AI Config's targeting — individual targets, rules and rollouts — through the " +
        ...
        build(args) {
            ...
            if ('error' in config)
                return config;
            return {
                path: `/api/v2/projects/${encodeURIComponent(project.key)}/ai-configs/` +
    ...
    },
    {
        ...
        beta: true,
        description: "Read one variation of an AI Config through the Butchr daemon's own credential. Read the " +
 ...
 */
export function ldOperationsFor(mode) {
    if (mode === 'off')
        return [];
    const on = new Set(enabledLdModes(mode));
 ...
 */
export function selectedLdProxyMode(env = process.env) {
    ...
    return {
        ...
        source: 'default',
        rawValue: raw,
        fallbackReason: `${LD_PROXY_ENV_VAR} is set to "${raw}", which is not one of ${LD_PROXY_MODES.join(' | ')}. ` +
...
export function ldProxyReport(decision, credential, now = () => new Date()) {
 ...
 */
export function refuseLdProxyCall(mode, tool) {
    if (mode === 'off') {
        return {
            reason: 'proxy-off',
            error: `The Butchr daemon's LaunchDarkly proxy is off, so ${tool} is refused. It is off by ` +
    ...
    if (!op) {
        ...
        const isKnownWrite = LD_WRITE_SCOPE_TOOLS.some((name) => tool === `launchdarkly_${name.replace(/-/g, '_')}`);
        return {
            reason: 'unknown-tool',
