 ...
 */
export function saveCostEstimate(measured, file = COST_ESTIMATE_PATH) {
 ...
 */
export function loadCostEstimate(file = COST_ESTIMATE_PATH, now = Date.now(), machineTotalBytes = os.totalmem()) {
    let parsed;
    try {
    ...
    if (!positive(residentBytes) || !positive(cores))
        return null;
    if (!positive(windowSeconds) || !positive(agentTrees))
        return null;
    ...
    const supervisorResidentBytes = positive(parsed.supervisorResidentBytes) &&
        parsed.supervisorResidentBytes <= machineTotalBytes
        ? parsed.supervisorResidentBytes
        : null;
    ...
    if (residentBytes > machineTotalBytes)
        return null;
    if (sampledAt > now + FUTURE_TOLERANCE_MS)
        return null;
    if (now - sampledAt > COST_ESTIMATE_MAX_AGE_MS)
        return null;
    return {
        residentBytes,
        cores,
 ...
 * to resurrect it.
 */
export function clearCostEstimate(file = COST_ESTIMATE_PATH) {
    try {
        fs.unlinkSync(file);
    }
    catch {
