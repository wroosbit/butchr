 ...
 * can carry.
 */
export function runHerdrCli(args) {
 ...
 */
export function paneIdForCwd(cwd) {
    const agents = runHerdrCli(['agent', 'list'])?.result?.agents;
    if (!Array.isArray(agents))
        return null;
    const here = agents.filter((a) => a?.cwd === cwd && typeof a?.pane_id === 'string');
    if (here.length === 0)
        return null;
    if (here.length > 1) {
        throw new Error(`herdr lists ${here.length} panes in ${cwd} (${here
            .map((a) => `'${a.name}'`)
