 ...
 * nobody could name.
 */
export function containWorkspaceDir(type, key) {
    ...
    return { outcome: 'contained', dir: dir };
}
/**
 ...
 * `router.ts` read.
 */
export function deleteWorkspaceDir(type, key) {
    ...
    if (containment.outcome === 'refused') {
        const error = `Refusing to reset workspace '${type}/${key}': ${containment.reason}. ` +
 ...
 */
export function clearWorkspaceMcpResidue(type, key, names) {
    if (names.length === 0)
    ...
    if (containment.outcome === 'refused') {
        return {
            outcome: 'refused',
            reason: `${containment.reason}. Only ${WORKSPACE_MCP_CONFIG} inside '${workspacesRoot()}' ` +
                `may be edited.`
        };
    }
    ...
    try {
        ...
        return {
            outcome: 'refused',
            reason: `${file} is not readable as JSON (${e?.message ?? String(e)}), so the entries to ` +
    ...
    if (!config || typeof config !== 'object' || Array.isArray(config)) {
        return {
            outcome: 'refused',
            reason: `${file} parses to ${Array.isArray(config) ? 'an array' : typeof config}, not a JSON ` +
