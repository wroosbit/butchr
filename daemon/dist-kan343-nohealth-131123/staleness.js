...
import { BUILD_SKEW_TOLERANCE_MS, classifyServerBuild, isBehindDeploy, newestMtimeMs } from './mcp-build.js';
/**
 ...
 */
function fetchHeadAgeMs(repoRoot, now) {
 ...
 */
function newestFile(root, skip = SKIP_DIRS) {
    let bestPath = '';
    let bestMtime = -1;
    let scanned = 0;
    const walk = (dir, rel) => {
...
const EXTENSION_BUILD_INPUTS = {
    ...
    notInputs: [
        ...
        {
            entry: 'scripts',
            because: 'verify scripts, render harnesses and screenshot drivers; they run against a build and no ' +
...
function matchesEntry(name, pattern) {
...
function classifyEntries(rootAbs, spec) {
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (spec.inputs.some((p) => matchesEntry(name, p)))
            inputs.push(name);
        else if (spec.notInputs.some((n) => matchesEntry(name, n.entry)))
            continue;
 ...
 * fragment of one.
 */
function newestAmong(rootAbs, names) {
    let best = null;
    let scanned = 0;
...
function describeInputs(spec, classified) {
 ...
 */
export function classifyRemoteRelation(repoRoot, remoteRef) {
 ...
 * a slower one.
 */
function checkGit(repoRoot, now) {
    const base = { id: 'git', label: 'local checkout' };
    ...
    if (!remoteRef) {
        return {
            ...
            detail: `${repoRoot} has no origin/HEAD and no origin/main ref. Run \`git -C ${repoRoot} fetch origin\` once ` +
    ...
    if (branch !== defaultBranch) {
        ...
        return {
            ...
            detail: `This checkout is on ${where} — a deliberate checkout, so it is not judged against ${remoteRef}. ` +
    ...
    if (relation.kind === 'shallow') {
        return {
            ...
            detail: `\`git rev-parse --is-shallow-repository\` is true, so this clone's history is truncated at a graft ` +
    ...
    if (relation.kind === 'diverged') {
        return {
            ...base,
            state: 'stale',
            headline: `${defaultBranch} has DIVERGED from ${remoteRef} — ` +
                `${relation.ahead} ahead, ${relation.behind} behind`,
            detail: `HEAD is ${head}, ${remoteRef} is ${remoteHead}. This checkout has ${relation.ahead} local ` +
    ...
    if (relation.kind === 'behind') {
        return {
            ...
            detail: `HEAD is ${head}, ${remoteRef} is ${remoteHead}: ${relation.behind} commit${relation.behind === 1 ? '' : 's'} merged that ` +
    ...
    if (fetchAgeMs === null || fetchAgeMs > FETCH_KNOWLEDGE_MAX_AGE_MS) {
        return {
            ...
            detail: `HEAD is ${head} and matches the ${remoteRef} this clone knows about, but ${provenance} — ` +
    ...
    return {
        ...
        detail: `HEAD is ${head}, the same commit as ${remoteRef}` +
 ...
 */
function checkPromptSource(repoRoot, now) {
    const base = { id: 'prompt-source', label: 'prompt source' };
    ...
    if (source.kind === 'worktree') {
        return {
            ...
            detail: `${source.because}. The working tree's default branch is never advanced — agents read it ` +
    ...
    if (ageMs === null) {
        return {
            ...
            detail: `${at}, and templates are read there rather than from the working tree — but there is no ` +
    ...
    if (ageMs > PROMPT_REF_MAX_AGE_MS) {
        return {
            ...
            detail: `${at}, and templates are read there rather than from the working tree, so briefs are current ` +
    ...
    return {
        ...
        detail: `${at}. Templates are read there with \`git show\`, so no working tree is opened and no lock is ` +
 ...
 */
function checkBuild(id, label, repoRoot, spec, distRel, remedy, now, note) {
    ...
    if (!src) {
        return {
            ...
            detail: `Nothing readable at the declared inputs of ${spec.root} (${inputSet}), so there is no source ` +
    ...
    if (!dist) {
        return {
            ...
            detail: `${distDir} is missing or empty, while ${spec.root} has ${src.scanned} input files, the newest being ` +
    ...
    if (lagMs > BUILD_SKEW_TOLERANCE_MS) {
        return {
            ...
            detail: `${spec.root}/${src.relPath} was modified ${ageOf(src.mtimeMs, now)} ago, ${describeAge(lagMs)} after ` +
    ...
    if (classified.unclassified.length) {
        ...
        return {
            ...base,
            state: 'unknown',
            headline: `${spec.root} holds ${classified.unclassified.length} entr${classified.unclassified.length === 1 ? 'y' : 'ies'} this check cannot classify`,
            detail: `${distRel} is newer than every declared input (${inputSet}), but ${names} ${classified.unclassified.length === 1 ? 'is' : 'are'} declared neither an input nor a non-input. If the build reads ${classified.unclassified.length === 1 ? 'it' : 'them'}, this item would report fresh over a ${distRel} that was behind ${classified.unclassified.length === 1 ? 'it' : 'them'}.`,
            remedy: `Classify ${names} in the BuildInputs for '${id}' in daemon/src/staleness.ts — as an input, or as a ` +
    ...
    return {
        ...
        detail: `${distRel} was last written ${ageOf(dist.mtimeMs, now)} ago (${dist.relPath}); the newest of the ` +
 ...
 */
function checkDaemonProcess(repoRoot, startedAt, now) {
    const base = { id: 'daemon-process', label: 'running daemon' };
    ...
    if (dist.mtimeMs > startedAt.getTime() + BUILD_SKEW_TOLERANCE_MS) {
        return {
            ...
            detail: `The daemon has been up ${upFor} (since ${startedAt.toISOString()}), but daemon/dist was rebuilt ` +
 ...
 */
function checkMcpServers(repoRoot, servers, now) {
    const base = { id: 'mcp-servers', label: 'agent MCP servers' };
    ...
    if (!servers) {
        return {
            ...
            detail: 'This staleness report was built without a connection registry, so which MCP servers are ' +
    ...
    if (servers.length === 0) {
        return {
            ...
            detail: `Nothing has announced itself to this daemon, so this is a statement about what can be seen ` +
    ...
    const describe = (server, relation) => {
        const who = `${server.type}/${server.key}`;
        const pid = server.build ? `pid ${server.build.pid}` : 'pid unknown';
        switch (relation.kind) {
            case 'older':
                return `${who} (${pid}, up since ${server.build?.startedAt}) loaded a build ${describeAge(relation.behindMs)} older than daemon/dist`;
            case 'unstamped':
    ...
    const undecidable = judged.filter((j) => j.relation.kind === 'other-tree' || j.relation.kind === 'unreadable');
    ...
    const remedy = 'Nothing here restarts them, deliberately: an mcp.js is a stdio child of its agent\'s client, so ' +
    ...
    if (behind.length) {
        return {
            ...base,
            state: 'stale',
            headline: `${behind.length} of ${servers.length} agent MCP server${servers.length === 1 ? '' : 's'} ${behind.length === 1 ? 'is' : 'are'} running a build older than daemon/dist`,
            detail: `daemon/dist was last written ${distAge} ago. ` +
    ...
    if (undecidable.length) {
        return {
            ...base,
            state: 'unknown',
            headline: `${undecidable.length} of ${servers.length} agent MCP server${servers.length === 1 ? '' : 's'} cannot be judged against daemon/dist`,
            detail: `No connected server is demonstrably behind daemon/dist (written ${distAge} ago), but ` +
    ...
    return {
        ...base,
        state: 'fresh',
        headline: `all ${servers.length} connected agent MCP server${servers.length === 1 ? '' : 's'} loaded the build on disk`,
        detail: `daemon/dist was last written ${distAge} ago and every connected server loaded it or later: ` +
 ...
 */
export function getStalenessReport(options) {
    ...
    const key = `${options.repoRoot}|${options.daemonStartedAt?.getTime() ?? ''}|${options.servers ? 'servers' : 'no-servers'}`;
    const now = Date.now();
    ...
    try {
        items = [
            ...
            checkBuild('daemon-build', 'daemon build', options.repoRoot, DAEMON_BUILD_INPUTS, 'daemon/dist', 'cd daemon && npm run build && restart the daemon', now),
            ...
            checkBuild('extension-build', 'extension build', options.repoRoot, EXTENSION_BUILD_INPUTS, 'extension/dist', 'cd extension && npm run build, then reload the extension at chrome://extensions', now, 
...
export function resetStalenessCache() {
    cached = null;
}
/** The report as lines for a log or a terminal, one item each. */
export function formatStalenessReport(report) {
    const mark = {
        fresh: 'OK  ',
        stale: 'STALE',
        unknown: '?   ',
    ...
    const lines = report.items.map((i) => `  ${mark[i.state]} ${i.label}: ${i.headline}\n        ${i.detail}` +
        (i.remedy ? `\n        fix: ${i.remedy}` : '') +
        (i.note ? `\n        note: ${i.note}` : ''));
    return [
        report.stale
