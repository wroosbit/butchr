...
import { resumeNudge } from './resume.js';
 ...
 */
export async function waitForAgentReady(herdrBridge, key, type) {
    ...
    for (;;) {
        ...
        if (tail.success && typeof tail.text === 'string') {
            ...
            if (AGENT_READY_MARKERS.some((marker) => text.includes(marker.toLowerCase())))
                return true;
        }
...
export async function nudgeResumedAgent(opts) {
    ...
    try {
        if (!(await waitForAgentReady(herdrBridge, key, type))) {
            log(`[nudge] ${label} restored its conversation but never reached a prompt within ` +
        ...
        // this module's.
        const sent = await herdrBridge.sendToAgent(key, resumeNudge(type, key, herdrBridge.briefLocation(type, key), cause), type);
        log(`[nudge] ${label} restored its conversation; ` +
            (sent.success
 ...
 */
export function deliveryFingerprint(message) {
 ...
 */
export function messageLanded(tail, message) {
 ...
 * rather than assumed.
 */
export function landedCount(tail, message) {
    const needle = deliveryFingerprint(message);
    if (!needle)
        return 0;
    const composerAt = COMPOSER_MARKERS.reduce((furthest, marker) => Math.max(furthest, tail.lastIndexOf(marker)), -1);
...
export async function deliverToAgent(opts) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        if (await confirmDelivered(herdrBridge, type, key, message, before, confirmTimeoutMs, pollMs)) {
            log(`[deliver] ${label}: delivered and confirmed on the pane` +
    ...
    log(`[deliver] ${label}: NOT delivered after 2 attempts (${lastError}). ` +
 ...
 */
async function readLandedCount(herdrBridge, type, key, message) {
...
async function confirmDelivered(herdrBridge, type, key, message, before, timeoutMs, pollMs) {
    ...
    for (;;) {
        ...
        if ((await readLandedCount(herdrBridge, type, key, message)) > before)
            return true;
        if (monotonicNow() >= deadline)
            return false;
        await delay(pollMs);
 ...
 * human interrupting.
 */
export function supervisionNudgeText(change, 
/**
...
runtimeName) {
    ...
    return (`${DAEMON_SENDER_TAG} ${change.type}/${change.key} (${change.agentName}), ` +
...
export class SupervisionNotifier {
     ...
     */
    async onSweep(census) {
        const result = { changes: [], delivered: 0, skipped: [] };
        try {
            const changes = this.recognise(census);
            result.changes = changes;
            const running = new Set(census.agents
                .filter((agent) => agent.type)
                .map((agent) => agentNameFor(agent.type, agent.key)));
            for (const change of changes) {
                const outcome = await this.notify(change, running);
                if (outcome === null)
                    result.delivered++;
                else
                    result.skipped.push({ change, reason: outcome });
            }
        }
        catch (e) {
    ...
    recognise(census) {
        ...
        for (const agent of census.agents) {
            if (!agent.type)
                continue; // Not addressable; nothing to notify about.
            ...
            if (previous !== 'working')
                continue;
            if (this.announced.has(`${agent.agentName}:blocked`))
                continue;
        ...
        for (const agentName of this.lastStatus.keys()) {
            if (!live.has(agentName))
                this.lastStatus.delete(agentName);
        }
        return changes;
    }
    /**
    ...
    async notify(change, running) {
        ...
        if (supervisorName === change.agentName) {
            log(`[supervision] ${subject}: its supervisor of record is itself; not telling an agent ` +
                `about its own status.`);
            return 'self-referential supervisor of record';
        }
        if (!running.has(supervisorName)) {
            log(`[supervision] ${subject}: supervisor ${supervisor.type}/${supervisor.key} is not running; ` +
