 ...
 */
export const VERIFIED_CLIENT_VERSIONS = ['2.1.224', '2.1.226'];
/**
...
export async function runChannelSelfCheck(opts) {
    ...
    if (first.outcome !== 'connection-replaced')
        return first;
...
async function attemptChannelSelfCheck(opts) {
    ...
    const done = (outcome, detail, extra = {}) => {
        ...
        world.log(`[ChannelSelfCheck] ${who}: ${outcome} → ${report.transport} ` +
        ...
        if (report.transport === 'composer') {
            ...
            world.log(`[ChannelSelfCheck] ${who}: this agent is ON THE COMPOSER — its sends will interrupt ` +
    ...
    if (opts.startupOutcome && opts.startupOutcome !== 'ready') {
        return done('not-ready', `channel startup supervision ended in '${opts.startupOutcome}', so there was no loop to ` +
    ...
    if (!world.emissionEnabled()) {
        return done('channel-disabled', 'channel emission was switched off before the check ran, so nothing was written');
    }
    const connection = world.resolveConnection();
    if (!connection) {
        return done('no-connection', 'no live connection is registered for this agent, so there was nothing to write the probe ' +
    ...
    if (!world.writeProbe(nonce)) {
        ...
        return done('no-connection', `the probe frame could not be written to ${connection.id}; the connection closed between ` +
            `resolving it and writing to it`, { connectionId: connection.id });
    }
    ...
    if (live && live.id !== connection.id) {
        return done('connection-replaced', `the probe frame was written to ${connection.id}, and by the time this check settled ` +
            ...
            (attempt === 1
                ...
                : `, and this attempt was already the re-run — two swaps inside one check, so nothing ` +
                    `further is tried. Re-activating this agent runs the check again`), { connectionId: connection.id });
    }
    if (!ack) {
        return done('no-answer', `the probe frame was written to ${connection.id} and this agent's MCP server did not ` +
            `answer within ${timeoutMs}ms. Either it is not running the build this daemon expects, ` +
            `or it is wedged: an addressed send to it would report success and prove nothing`, { connectionId: connection.id });
    }
    ...
    const clientVersionVerified = clientVersion === null ? null : VERIFIED_CLIENT_VERSIONS.includes(clientVersion);
    const carried = {
    ...
    if (!ack.emitted) {
        return done('emit-failed', `this agent's MCP server could not emit notifications/claude/channel to its client: ` +
            `${ack.emitError ?? 'no reason given'}. The channel contract has moved under us or the ` +
            `client's transport is gone`, carried);
    }
    if (!ack.pingAnswered) {
        return done('client-unresponsive', `the channel frame was written to the client's transport and the client did not answer an ` +
            `MCP ping behind it (${ack.pingError ?? 'no reason given'}), so nothing here can say it was ` +
            `read. A client that has stopped reading its stdin looks exactly like this`, carried);
    }
    if (clientVersion === null) {
        ...
        // this is not `passed`.
        return done('unverified-client', `the loop crossed to a client that answered a ping, and that client reported no version in ` +
            `its initialize handshake, so this result is pinned to nothing`, carried);
    }
    if (!clientVersionVerified) {
        return done('unverified-client', `the loop crossed and the client read the frame — but ${clientName ?? 'this client'} ` +
            ...
            `assumes otherwise`, carried);
    }
    return done('passed', `the channel loop crossed to ${clientName ?? 'the client'} ${clientVersion} and the client ` +
        `read it — a notifications/claude/channel frame was emitted on connection ${connection.id} ` +
        `and an MCP ping ordered behind it on the same stream was answered`, carried);
}
/**
