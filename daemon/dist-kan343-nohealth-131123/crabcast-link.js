...
export class CrabCastLink {
    ...
    previousPeerBootId = null;
    peerIdentityVerdict = 'unknown';
    /**
     ...
     */
    handshakeSettled = Promise.resolve('unknown');
    settleHandshake = () => { };
    socketPath;
    requestTimeoutMs;
    reconnectDelayMs;
    ...
    connect() {
        ...
        socket.on('connect', () => {
            ...
            this.handshakeSettled = new Promise((resolve) => {
    ...
    async handshake() {
        try {
            ...
            if (current !== null && previous !== null && this.peerIdentityVerdict === 'restarted') {
                this.log(`peer bootId moved ${previous.slice(0, 8)} → ${current.slice(0, 8)}: ` +
    ...
    emitLinkState(event) {
        for (const handler of this.linkStateHandlers) {
            try {
                ...
                this.log(`a link-state handler threw on ${event.state}: ` +
