 ...
 */
export function isFdCeilingUnraised(usage) {
    return usage.softLimit <= FD_SETSIZE;
}
/** One line naming the ceiling in panes, and where the permanent fix lives. */
export function describeFdCeiling(usage) {
    return (`herdr server (pid ${usage.pid}) has an open-file soft limit of ${usage.softLimit}, the stock default. ` +
 ...
 */
export function findHerdrServerPid() {
    let entries;
    try {
 ...
 */
export function readFdUsage(pid = findHerdrServerPid()) {
    if (pid === undefined)
        return undefined;
 ...
 * anything polled.
 */
export function readFdPressure(pid = findHerdrServerPid()) {
    ...
    for (const fd of fdNames) {
        try {
            if (fs.readlinkSync(`/proc/${usage.pid}/fd/${fd}`) === '/dev/ptmx')
                ptmxFds++;
        }
        catch {
...
export function describeFdPressure(p) {
    const percent = Math.round(p.ratio * 100);
    return (`herdr server (pid ${p.pid}) holds ${p.openFds}/${p.softLimit} open files (${percent}% of the soft limit); ` +
...
export function isFdPressureHigh(p) {
...
export function parseHerdrVersion(versionOutput) {
 ...
 */
export function checkHerdrVersion(versionOutput) {
    ...
    if (major > wantMajor || (major === wantMajor && minor > wantMinor)) {
        return (`${versionOutput.trim()} is newer than the ${SUPPORTED_HERDR_MAJOR_MINOR}.x line Butchr's spawn path ` +
    ...
    return (`${versionOutput.trim()} is older than the ${SUPPORTED_HERDR_MAJOR_MINOR}.x line Butchr is written ` +
 ...
 */
export function diagnoseSpawnFailure(herdrMessage) {
    const notes = [];
    ...
    if (/ghostty error -2/i.test(herdrMessage)) {
        notes.push('This is a pane-geometry failure, not a resource failure: herdr tried to create the pane ' +
    ...
    if (/too many open files|no file descriptors available|EMFILE|ENFILE|openpty/i.test(herdrMessage)) {
        notes.push('The herdr server is out of file descriptors. Each pane costs ' +
