...
export function isSupervisorType(type) {
...
export class WorkspaceRegistry {
     ...
     */
    constructor(state = new IntegrationStateStore()) {
        this.state = state;
    }
    /**
    ...
    registerIntegration(integration) {
        ...
        if (this.state.decideIfUndecided(integration.id, configured)) {
            console.log(`[Integrations] ${integration.id}: no enabled state on record; ` +
                (configured
    ...
    mcpServerDefinitions() {
        ...
        for (const integration of this.integrationsById.values()) {
            if (!integration.enabled)
                continue;
            if (!integration.mcpServers)
                continue;
            if (integration.credential && !integration.credential.status().configured)
                continue;
     ...
     */
    disabledIntegrationForType(type) {
        for (const integration of this.integrationsById.values()) {
            if (integration.enabled)
                continue;
            if (integration.workspaceTypes.some((config) => config.type === type))
                return integration;
        }
        return null;
    }
    ...
    async resolve(url) {
        ...
        if (!refinedType || refinedType === matched.config.type)
            return matched;
