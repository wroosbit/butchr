...
import { classifyStartupDialog } from './startup-dialog.js';
/**
 ...
 */
export const DEV_CHANNELS_DIALOG_PATTERN = /Loading development channels|I am using this for local development/;
/**
...
export async function superviseChannelStartup(opts) {
    ...
    const done = (outcome, connectionId, detail) => ({
        outcome,
        ready: outcome === 'ready',
        dialogsAnswered,
        connectionId,
    ...
    while (world.now() < deadline) {
        ...
        if (pane === null) {
            ...
            // screen.
            const what = dialog.kind === 'foreign'
            ...
            const detail = `${what} is on the pane, not the development-channels one, so NOTHING WAS PRESSED ` +
            ...
            if (!undelimitedLogged) {
                ...
                world.log(`[ChannelStartup] ${who}: the development-channels prose is on the pane but no ` +
            ...
            if (dialogsAnswered >= MAX_DIALOG_ANSWERS) {
                if (!capReached) {
                    capReached = true;
                    world.log(`[ChannelStartup] ${who}: ${MAX_DIALOG_ANSWERS} dialogs answered and another is on ` +
                        `screen — refusing to press Enter again at a startup sequence this no longer models`);
                }
            }
            else {
                world.log(`[ChannelStartup] ${who}: development-channels dialog #${dialogsAnswered + 1} — ` +
                ...
                try {
                    ...
                    world.log(`[ChannelStartup] ${who}: could not send Enter to the pane, will retry: ` +
                        `${e?.message ?? String(e)}`);
                }
            ...
            if (connection && atPrompt) {
                ...
                world.log(`[ChannelStartup] ${who}: ready after ${waited}ms — the agent is at its prompt and ` +
                    `its MCP server is registered (connection ${connection.id}, ` +
                    `${dialogsAnswered} dialog(s) answered)`);
                return done('ready', connection.id, `the agent reached its prompt and its MCP server registered with the daemon ` +
    ...
    if (dialogOnScreen) {
        const detail = `a development-channels dialog was still on the pane ${waited}ms after spawn ` +
    ...
    if (paneFailures === paneReads) {
        const detail = `the pane could not be read at all in ${paneReads} attempt(s) over ${waited}ms, so ` +
    ...
    if (sawConnection && !atPrompt) {
        const detail = `an MCP server registered for this agent but the pane never reached a session prompt ` +
    ...
    if (sawDialog && !sawConnection) {
        const detail = `a development-channels dialog was raised and nothing ever connected in the ` +
    ...
    const detail = `no MCP server registered for this agent within ${waited}ms of spawn ` +
 ...
 */
function logRevert(log, who) {
    log(`[ChannelStartup] ${who}: REVERT — turn channels off with ` +
 ...
 */
export function freshConnectionFrom(registry, address) {
    const GRACE_MS = 1000;
    return (spawnedAt) => {
        const connection = registry.resolve(address);
        if (!connection)
            return null;
        if (connection.registeredAt.getTime() + GRACE_MS < spawnedAt)
            return null;
