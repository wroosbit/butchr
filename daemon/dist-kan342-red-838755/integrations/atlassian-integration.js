import { PRIORITY_EPIC, PRIORITY_STORY, PRIORITY_TASK } from '../priority.js';
 ...
 */
const WORKSPACE_TYPE_BY_JIRA_ISSUE_TYPE = {
    epic: 'epic',
    story: 'story',
...
export function workspaceTypeForJiraIssueType(issueTypeName) {
    if (!issueTypeName)
        return DEFAULT_JIRA_WORKSPACE_TYPE;
    return (WORKSPACE_TYPE_BY_JIRA_ISSUE_TYPE[issueTypeName.trim().toLowerCase()] ??
 ...
 * not let you ignore.
 */
export function workspaceTypeForJiraIssueTypeStrict(issueTypeName) {
    if (!issueTypeName)
        return null;
 ...
 */
export function jiraIssueWorkspaceTypes() {
 ...
 */
export function atlassianMcpServers() {
 ...
 * out.
 */
function pageIdFromTinyId(tinyId) {
 ...
 */
const CONFLUENCE_PAGE_URL_FORMS = [
 ...
 */
export function atlassianWorkspaceTypes(lookup) {
    return [
        {
            type: 'task',
            name: 'Jira Task',
            ...
            ...(lookup
                ? {
                    refine: async (key) => workspaceTypeForJiraIssueType(await lookup(key))
                }
                : {})
        },
 ...
 */
export function createAtlassianIntegration(options = {}) {
    return {
