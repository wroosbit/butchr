 ...
 * displace another.
 */
export function outranks(incoming, running) {
 ...
 */
export function compareVictims(a, b) {
 ...
 * epic agents.
 */
export function selectVictim(candidates, incoming) {
 ...
 */
export function describeFleetPriorities(candidates) {
    if (candidates.length === 0)
        return 'nothing is running that could be stood down';
 ...
 */
export function preemptionOffer(victim, incoming) {
    return (`Standing down ${describeCandidate(victim)} would make room: this activation is ` +
...
export function noVictimReason(candidates, incoming) {
    return (`Nothing running is below priority ${incoming}, so there is nothing this ` +
