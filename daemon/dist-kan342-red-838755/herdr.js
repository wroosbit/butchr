...
import { RESUME_ENV, degradedResumePrompt, hasRestorableConversation } from './resume.js';
...
export function workspaceDirFor(type, key) {
...
export class HerdrBridge {
    ...
    initPty(session, initialPrompt, defaultAgent, mcpServers) {
        ...
        if (mcpServers && Object.keys(mcpServers).length > 0) {
            writeWorkspaceMcpConfig(session.workDir, withWorkspaceIdentity(mcpServers, { type: session.type, key: session.key }));
        }
        ...
        if (!agentExists) {
            ...
            const fallbackPrompt = session.resume && session.resumedConversation === false
    ...
    runHerdr(args) {
        ...
        if (reported) {
            const error = new Error(reported.message ?? `herdr reported ${reported.code ?? 'an error'}`);
            ...
            if (typeof reported.code === 'string')
                error.herdrCode = reported.code;
            throw error;
        }
        if (result.status !== 0) {
    ...
    getSessionByAddress(key, type) {
        const trimmedType = typeof type === 'string' ? type.trim() : '';
        if (!trimmedType)
            return this.getSessionByKey(key);
        for (const session of this.sessions.values()) {
    ...
    resetWorkspace(type, key) {
        ...
        const refuse = (reason) => {
            const error = `Refusing to reset workspace '${type}/${key}': ${reason}. ` +
