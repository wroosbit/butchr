...
import { agentNameFor, workspaceDirFor, workspacesRoot } from './herdr.js';
/**
 ...
 */
export function readUnreadableDisclosure(frame) {
...
export class CrabCastRuntime {
     ...
     */
    setAgentSpawnedListener(_listener) {
        this.log('setAgentSpawnedListener: registered and never fired. The spawn verdict IS available ' +
     ...
     */
    spawnSession(type, key, url, promptContent, defaultAgent, mcpServers, resume) {
    ...
    async provision(session, promptContent, defaultAgent, mcpServers) {
        const configure = {
    ...
    resetWorkspace(type, key) {
        return {
            success: false,
            error: renderRefusal(this.link.refusal('butchr-adapter', `resetWorkspace(${type}/${key}) has no CrabCast counterpart: their \`reset\` was ` +
                'removed deliberately, because CrabCast never creates the directory an agent runs ' +
                'in and so may not delete one', 'Deleting the workspace becomes Butchr\'s own job under this runtime. That wiring is ' +
    ...
    pressPaneKey(key, type, keyName) {
        throw new Error(renderRefusal(this.link.refusal('butchr-adapter', `pressPaneKey(${type ?? '*'}/${key}, ${keyName}) has no CrabCast counterpart — ` +
            'there is no press_pane_key action, and send_to_agent opens with a Ctrl+C, which ' +
            'is the one thing this method must not do', 'This is an interface observation for KAN-59, not a change request. Channel startup ' +
    ...
    sessionForKey(key) {
        return [...this.sessions.values()].find((s) => s.key.toLowerCase() === key.toLowerCase() && s.status !== 'terminated');
    }
    /**
