...
import { MEASURED_AGENT_COST, sampleCpuBusy, setMeasuredAgentCost } from './capacity.js';
...
const channelLiveness = new ChannelLivenessProbe({
    world: {
        ...
        candidates: () => agentConnections.addresses().flatMap((address) => {
            ...
            if (channelSelfChecks.degraded(address))
                return [];
...
const server = net.createServer((socket) => {
    ...
    const router = new MessageRouter(registry, promptLoader, herdrBridge, (msg) => writeJsonLine(socket, msg), broadcast, {
        ...
        channelCarrier: (address, degraded) => carrierFor({
            emissionEnabled: channelEmissionEnabled(),
            degraded,
...
function sampleFleetCost() {
    ...
    if (!sample) {
        degradeCostMeasurement(measurement.chargeable.agents <= 0
