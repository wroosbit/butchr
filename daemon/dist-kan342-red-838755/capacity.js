 ...
 */
export function unobservedStartsAmong(startedAt, measured) {
    if (!measured)
        return { count: startedAt.length, because: 'no-measurement' };
...
export function describeCapacity(c) {
    ...
    if (c.measured) {
        ...
        lines.push(`  ${restored ? 'restored (damped)' : 'measured (damped)'}: ` +
...
export function summarizeCapacity(c) {
    ...
    const figures = `${c.running}/${c.cap} task agents, room for ${c.headroom} more ` +
