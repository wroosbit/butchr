 ...
 */
export function explainAbsence(key, diagnostic, accountId) {
...
export class BoardReconciler {
    ...
    async readDiagnostic() {
        try {
            ...
            this.opts.log(`[board] the diagnostic query threw: ${e?.message ?? String(e)}. Convergence is ` +
    ...
    report(diff, mode, cycle, standDowns) {
        ...
        for (const { agent, reason } of cycle.spared) {
            this.opts.log(`[board] ${address(agent)} is running and \`${this.jql}\` did not return it, but ` +
