 ...
 */
export function templateProvenance(repoRoot, templatePath, now = new Date()) {
    const base = {
        repoRoot,
        templatePath,
        renderedAt: now,
...
export function renderProvenanceBlock(p) {
    ...
    lines.push(`- **\`${p.templatePath}\` last changed in \`${shortSha}\`** — *${subject}* (${date}).`, `- **To find out whether that is still the rule**, run these two — they are the` +
        ` whole check:`, '', '  ```bash', `  git -C ${p.repoRoot} fetch origin`, `  git -C ${p.repoRoot} log --oneline ${shortSha}..origin/main -- ${p.templatePath}`, '  ```', '', `  **No output means this brief is current** and you need do nothing else. ` +
