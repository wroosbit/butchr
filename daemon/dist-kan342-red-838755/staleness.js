...
function checkGit(repoRoot, now) {
    ...
    if (behind > 0) {
        return {
            ...
            detail: `HEAD is ${head}, ${remoteRef} is ${remoteHead}: ${behind} commit${behind === 1 ? '' : 's'} merged that ` +
 ...
 */
function checkBuild(id, label, repoRoot, srcRel, distRel, remedy, now, note) {
    ...
    if (!dist) {
        return {
            ...
            detail: `${distDir} is missing or empty, while ${srcRel} has ${src.scanned} files, the newest being ` +
    ...
    if (lagMs > BUILD_SKEW_TOLERANCE_MS) {
        return {
            ...
            detail: `${srcRel}/${src.relPath} was modified ${ageOf(src.mtimeMs, now)} ago, ${describeAge(lagMs)} after the ` +
...
export function getStalenessReport(options) {
    ...
    try {
        items = [
            ...
            checkBuild('daemon-build', 'daemon build', options.repoRoot, 'daemon/src', 'daemon/dist', 'cd daemon && npm run build && restart the daemon', now),
            ...(options.daemonStartedAt
                ? [checkDaemonProcess(options.repoRoot, options.daemonStartedAt, now)]
                : []),
            checkBuild('extension-build', 'extension build', options.repoRoot, 'extension', 'extension/dist', 'cd extension && npm run build, then reload the extension at chrome://extensions', now, 
