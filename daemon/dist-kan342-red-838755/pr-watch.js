...
export function describeHealth(health, now) {
    if (!health.repos.length) {
        return ('No repository is being watched: no live agent holds a checkout with a GitHub ' +
    ...
    if (!health.lastAttemptAt) {
        return (`Watching ${health.repos.join(', ')}; the first look has not run yet, so nothing is ` +
    ...
    const asOf = `As of the last successful look, ${health.watchedCount} pull request(s) matched a ticket, ` +
 ...
 */
export function prEventNoticeText(events, relation) {
    ...
    const clause = (event) => {
        switch (event.kind) {
            ...
            case 'green-idle':
                return ('has every other check green and NO approval recorded at this head, so nothing is ' +
            ...
            case 'head-stale':
                return (`is ${event.mergeStateStatus ?? 'not mergeable'} against main — on a strict branch an ` +
...
export class PrWatcher {
    ...
    recognise(pr, issueKey) {
        ...
        const greenIdle = pr.state === 'OPEN' && !pr.isDraft && pr.checks === 'success' && !approved && !stale;
        if (!seen) {
            this.state.set(id, {
                state: pr.state,
