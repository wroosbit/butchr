 ...
 */
export function dampCost(previous, sample) {
    const step = (prev, next) => prev + (next > prev ? ALPHA_UP : ALPHA_DOWN) * (next - prev);
    return {
 ...
 */
export function sampleFromMeasurement(m, machineTotalBytes) {
    if (!Number.isFinite(m.elapsed) || m.elapsed <= 0)
        return null;
    const chargeable = m.chargeable;
    if (!chargeable || !Number.isFinite(chargeable.agents) || chargeable.agents <= 0)
        return null;
    const all = m.totals;
    if (!all || !Number.isFinite(all.agents) || all.agents <= 0)
        return null;
 ...
 */
export function supervisorMemoryFromMeasurement(m, machineTotalBytes) {
    const s = m.supervisors;
    if (!s || !Number.isFinite(s.agents) || s.agents <= 0)
        return null;
