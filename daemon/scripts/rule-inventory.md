...
- `H-30` — KAN-471 — a long ticket's comment history is a window and its completeness fields sit after the array that hides them: `getJiraIssue` returns 100 of KAN-39's 211 comments and the JQL route 20, both reporting `total`/`maxResults`/`startAt` correctly at 99.3% of the payload; parse the saved JSON rather than grepping it, because each field occurs once near the end and a partial read returns zero of them; read the container before the comments, and no route on the official MCP pages back to the rest
