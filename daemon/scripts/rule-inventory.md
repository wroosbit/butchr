...
- `H-30` — KAN-471 — a long ticket's comment history is a window: `getJiraIssue` returns 100 of KAN-39's 211 comments and the JQL route 20; parse the saved JSON rather than grepping it, because each completeness field occurs once near the end of the payload and a partial read returns zero of them; and on the second envelope, which strips the container entirely, an absent `total` means the read is not self-describing rather than that the ticket is short — no route on the official MCP pages back to the rest
