...
- `H-33` — KAN-651, absorbing KAN-665 — the seam question is about reads rather than writes: does either task make a claim ABOUT an artifact the other can change — a gate, a proof, a count — so a gate and the thing it gates are coupled with zero overlap in the diff, two things that merely queue for the same gate are coupled too, and **file-overlap checking cannot surface any of it**, which is why two deliberate reviews came back clean and confident on KAN-607/KAN-618; a ticket's stated scope is a forecast of what an agent will write rather than an observation of it; and the generated file is the seam a decomposition forgets, where a **clean** merge is not evidence the file is correct because a text merge preserves rows and cannot recompute derived values — only regeneration is
- `H-34` — KAN-651, absorbing KAN-665 — the limit claim: any statement that takes away someone's reason to check needs the evidence of a positive claim, because a false positive is tested by whoever acts on it and a false limit never is; its reverse form wears abundance rather than scarcity — *we can always get another* removes the reason to check exactly as *we cannot* does, and is harder to see because it sounds like confidence; when you retract a limit, re-state the hold on its real grounds or the retraction reads as a green light; and over-attribution is not humility either — anything time-ordered comes from the changelog, never from recollection
- `H-35` — KAN-651 — review does not only flow downward: a task agent's objection is the finding rather than insubordination, and the corollary that keeps it from teaching suspicion downward and credulity upward — a correction from below is not self-verifying, and specific, confident and well-formatted are the properties this brief treats as warning signs everywhere else
- `H-36` — KAN-651, from KAN-665, found by `story/KAN-611` — the re-derive window: re-check **between** the derivation and the transition that depends on it, not only before the derivation — the bullet above it was followed exactly and the read still landed in a twelve-second gap (changelog `14548` → `14549`) and was false sixty-one seconds later, so read your children in the same breath as your own transition
