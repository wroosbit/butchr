...
const router = new MessageRouter(
  ...
  () => {},
  () => {},
  // `capacitySource` is named, not counted. It used to be the tenth positional
  // argument in a tail of seven optional ones, and this file carried a §0 that
  // asked the router for capacity and refused to report any verdict unless the
  // injection had visibly taken effect — because an eleventh parameter inserted
  // after `launchdarkly` would have silently bound this argument to the wrong
  // slot, dropped §1 back onto the real machine, and left every section still
  // passing. KAN-226 made the tail an options object; that failure is now a
  // compile error in a checked caller and a constructor throw in an unchecked
  // one, so §0 was deleted with the hazard it watched. See KAN-226 for the
  // argument, and `MessageRouterOptions` in daemon/src/router.ts for the shape.
  { agentRegistry: new AgentRegistry(path.join(scratch, 'agents.jsonl')), capacitySource }
);
