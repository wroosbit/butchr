...
/**
 * Why a sampling window produced no usable figure.
 *
 * The two kinds are not degrees of the same thing:
 *
 *   - `instrument-failed` — something that should have worked did not, so the
 *     estimate may be wrong and nothing can say by how much.
 *   - `nothing-to-measure` — no task-agent tree existed to measure. Nothing is
 *     wrong with the last measurement; there is simply no new one, and an empty
 *     fleet is evidence about how busy the machine is rather than about what an
 *     agent costs on it.
 *
 * `reason` is the operator-facing sentence, logged verbatim.
 */
export type MeasurementGap =
  | { kind: 'instrument-failed'; reason: string }
  | { kind: 'nothing-to-measure'; reason: string };

/**
 * What the sampler should do about it.
 *
 * `retain` republishes the figure the daemon already holds, relabelled — the
 * arithmetic does not change, the label does, and the label is what a reader
 * judges the figure by.
 */
export type SamplerDecision =
  | { action: 'degrade'; reason: string }
  | { action: 'retain'; measured: MeasuredAgentCost; ageMs: number; reason: string };
...
export function staleMeasurementMaxAgeMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.BUTCHR_STALE_COST_MAX_MINUTES;
  if (raw === undefined || raw.trim() === '') return STALE_MEASUREMENT_MAX_AGE_MS;
 ...
 */
export function decideOnMissingSample(
  gap: MeasurementGap,
  held: MeasuredAgentCost | null,
  now: number,
  maxAgeMs: number = STALE_MEASUREMENT_MAX_AGE_MS
): SamplerDecision {
  switch (gap.kind) {
    ...
    case 'nothing-to-measure': {
      ...
      if (!held) return { action: 'degrade', reason: gap.reason };
      ...
      if (ageMs > maxAgeMs) {
        return {
          action: 'degrade',
          reason:
            `${gap.reason}, and the last measurement is ${Math.round(ageMs / 60000)} minutes old — ` +
