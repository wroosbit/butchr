...
/** Who the daemon believes is asking, as read off the request. */
 ...
 */
export function senderTagFor(identity: SenderIdentity | null | undefined): string {
  const type = identity?.type?.trim();
  const key = identity?.key?.trim();
  if (!type || !key) return UNIDENTIFIED_SENDER_TAG;
 ...
 */
export function withSenderTag(tag: string, message: string): string {
 ...
 */
export function hasLeadingTag(message: string, tag: string): boolean {
