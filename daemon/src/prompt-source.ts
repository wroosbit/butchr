...
/**
 * Where the loader read a template from — a shape, not a boolean.
 *
 * A boolean `fromRef` would let the provenance block claim a ref-based read
 * while the loader had quietly fallen back to the working tree, and that is
 * precisely the failure worth making unrepresentable: the block's whole job is
 * to tell an agent what its brief is a snapshot *of*, so a source it gets wrong
 * is worse than no block. As a union the renderer must switch on `kind`, and a
 * fallback carries the reason it happened into the text an agent reads.
 *
 * This follows `RemoteRelation` in `staleness.ts` for the same reason KAN-437
 * gave there: every case that needs different words is its own kind, and adding
 * a kind without handling it is a compile error rather than confident wrong
 * prose.
 */
export type PromptSource =
  /** Read at a fetched remote ref. The working tree is never opened. */
  | { kind: 'ref'; ref: string; sha: string }
  /** Read off the checkout's working tree, with why the ref was not usable. */
  | { kind: 'worktree'; because: string };
 ...
 */
export function resolvePromptSource(repoRoot: string, ref: string = PROMPT_REF): PromptSource {
 ...
 */
export function readTemplateAt(
  repoRoot: string,
  source: PromptSource,
  templatePath: string
): string | null {
 ...
 */
export function describeBuildGap(repoRoot: string, source: PromptSource): string | null {
  if (source.kind !== 'ref') return null;
  ...
  return (
    `${source.ref} is ${ahead} commit${ahead === 1 ? '' : 's'} ahead of this checkout's HEAD, ` +
...
export class PromptSourceKeeper {
  ...
  constructor(
    ...
    private readonly intervalMs: number = FETCH_INTERVAL_MS
  ) {}
   ...
   * using.
   */
  public fetchOnce(): Promise<void> {
    if (this.running) return Promise.resolve();
    this.running = true;
    return new Promise<void>((resolve) => {
      execFile(
        'git',
        ['-C', this.repoRoot, 'fetch', '--quiet', 'origin'],
        {
        ...
        (err) => {
          ...
          if (err) this.log(`prompt-source: fetch of ${this.repoRoot} failed: ${err.message}`);
          resolve();
        }
      );
    });
