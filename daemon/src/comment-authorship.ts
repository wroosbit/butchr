...
/** One comment, attributed. */
export interface CommentAuthor {
  /** The agent name, e.g. `butchr-task-kan-186`. */
  agentName: string;
  /** The issue as the tool call spelled it. Diagnostic; ids are unique. */
  issueKey: string;
  /** When the attribution was recorded, for the TTL. */
  at: number;
}

/** An agent, as this module needs to see one. */
export interface AuthorshipAgent {
  agentName: string;
  type: string;
  key: string;
}

export interface CommentAuthorshipOptions {
  /**
   * Where an agent's transcripts live. Injectable so a proof can point at a
   * temporary directory instead of the machine's real `~/.claude`.
   */
  transcriptDirFor?: (agent: AuthorshipAgent) => string;
  log?: (...args: any[]) => void;
  now?: () => number;
  seedTailBytes?: number;
  ttlMs?: number;
}

/** Where a file was last read up to, and any half-line left over. */
interface Cursor {
  offset: number;
  carry: string;
}
...
export class CommentAuthorship {
  private readonly authors = new Map<string, CommentAuthor>();
  private readonly pending = new Map<string, PendingWrite>();
  private readonly cursors = new Map<string, Cursor>();

  private readonly transcriptDirFor: (agent: AuthorshipAgent) => string;
  private readonly log: (...args: any[]) => void;
  private readonly now: () => number;
  private readonly seedTailBytes: number;
  private readonly ttlMs: number;

  constructor(opts: CommentAuthorshipOptions = {}) {
   ...
   */
  public scan(agents: AuthorshipAgent[]): void {
    for (const agent of agents) {
      ...
      for (const name of names) {
        try {
          ...
          this.log(
            `[authorship] could not read ${path.join(dir, name)}: ${e?.message ?? String(e)}`
          );
        }
      }
    }
  ...
  public authorOf(commentId: string): string | undefined {
   ...
   */
  private readAppended(file: string, agent: AuthorshipAgent): void {
  ...
  private readRange(file: string, from: number, to: number): string {
   ...
   */
  private absorb(record: any, agent: AuthorshipAgent): void {
    ...
    for (const block of content) {
      ...
      if (block.type === 'tool_use') {
        if (typeof block.name !== 'string' || !block.name.endsWith(COMMENT_TOOL_SUFFIX)) continue;
        const issueKey = block.input?.issueIdOrKey;
        if (typeof block.id !== 'string' || typeof issueKey !== 'string') continue;
      ...
      if (!commentId) {
        this.log(
          `[authorship] ${write.agentName} wrote a comment on ${write.issueKey} but its id ` +
      ...
      this.log(
        `[authorship] ${write.agentName} wrote comment ${commentId} on ${write.issueKey}.`
      );
    }
  }
  ...
  private prune(): void {
    ...
    for (const [id, author] of this.authors) {
      if (author.at < cutoff) this.authors.delete(id);
    }
    for (const [id, write] of this.pending) {
      if (write.at < cutoff) this.pending.delete(id);
    }
 ...
 */
export function createdCommentId(content: any): string | null {
