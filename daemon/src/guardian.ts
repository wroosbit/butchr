...
/** The guardian, as it is stored on disk. */
export interface GuardianConfig {
  /** The one guardian. `null` is the ordinary state and means none is set. */
  address: AgentAddress | null;
  /** Override for {@link DEFAULT_POKE_INTERVAL_MS}, or null for the default. */
  intervalMs: number | null;
  /** The sender tag of whoever set it — an agent, or the options page. */
  setBy: string | null;
...
export function noGuardian(): GuardianConfig {
 ...
 */
export function readGuardianConfig(readFile: () => string = defaultRead): GuardianConfig {
  try {
    const parsed = JSON.parse(readFile());
    const address = normaliseAddress(parsed?.address);
    if (!address) return noGuardian();
    const intervalMs =
...
function normaliseAddress(value: any): AgentAddress | null {
...
export type GuardianSetRefusal = 'already-set' | 'invalid-address';

/** What one attempt to set the guardian did. */
export type GuardianSetResult =
  | { ok: true; config: GuardianConfig; replaced: AgentAddress | null; detail: string }
  | { ok: false; refusal: GuardianSetRefusal; detail: string; incumbent: AgentAddress | null };
...
export function setGuardian(opts: {
  ...
  now: () => number;
  read: () => GuardianConfig;
  write: (config: GuardianConfig) => void;
}): GuardianSetResult {
  ...
  if (!address) {
    return {
      ...
      detail:
        'a guardian is a workspace type and key, e.g. type "epic" and key "KAN-203". Both must ' +
  ...
  const sameAsIncumbent =
    incumbent !== null && canonical(incumbent) === canonical(address);
  ...
  if (incumbent && !sameAsIncumbent && opts.replace !== true) {
    return {
      ...
      refusal: 'already-set',
      detail:
        `${describeAddress(incumbent)} is already the guardian` +
  ...
  return {
    ...
    detail:
      `${describeAddress(address)} is the guardian` +
      (sameAsIncumbent
...
export function clearGuardian(opts: {
  read: () => GuardianConfig;
  write: (config: GuardianConfig) => void;
}): { cleared: AgentAddress | null; detail: string } {
  const current = opts.read();
...
/** What one poke did. Ordered worst-to-best for a reader. */
export type GuardianPokeOutcome =
  /** A poke was asked for while one was in flight. Never recorded. */
  | 'already-running'
  /** No guardian is configured, so there was nobody to poke. */
  | 'no-guardian'
  /** Channel emission is off fleet-wide; nothing was written to anybody. */
  | 'channel-disabled'
  /**
   * The channel would not carry it, so **nothing was delivered**.
   *
   * THE OUTCOME AC2 IS ABOUT, and it is spelled `undelivered` rather than
   * `failed` on purpose: it is a fact about a frame, and the reader most likely
   * to misread it is one looking for a reassuring word. It covers every refusal
   * `carrierFor` can produce — an agent that is not running, one whose
   * registration was lost to a restart, one degraded to the composer — and
   * {@link GuardianPokeResult.reason} is what tells them apart.
   */
  | 'undelivered'
  /** The frame was written to a live channel connection. C1 and C2, no more. */
  | 'delivered';

/** One poke's record, as it is stored and as a supervisor reads it. */
export interface GuardianPokeResult {
  outcome: GuardianPokeOutcome;
  /**
   * The one field a caller should branch on.
   *
   * True only for `delivered`, and it is a claim about **a frame reaching a
   * connection** — never about the fleet being supervised. See this module's
   * header: nothing here is named for health, and this is the field that would
   * have been called `healthy` by an author who had not read it.
   */
  delivered: boolean;
  address: AgentAddress | null;
  connectionId: string | null;
  /**
   * The carrier, named the way `send_to_agent` names it.
   *
   * Only ever `'channel'` or `'undelivered'` — **there is no composer value and
   * there cannot be one**, because {@link GuardianWorld} has no composer in it.
   * That is the field a reader checks to see that a poke is not costing its
   * recipient a turn, and it is stated rather than left to be inferred from the
   * absence of a complaint.
   */
  transport: 'channel' | 'undelivered';
  /**
   * Always `false`, and said out loud rather than omitted.
   *
   * A poke every thirty minutes over the composer would be 48 destroyed tool
   * calls a day. Nothing here can interrupt: a channel frame is acted on at the
   * recipient's turn boundary, and a refused one is not retried on a carrier
   * that would. A reader should be able to see that from one poke's record
   * without having read this module.
   */
  interrupted: false;
  /** Why it was not delivered, verbatim from the channel's own refusal. */
  reason: string | null;
  startedAt: string;
  elapsedMs: number;
...
/**
 * What the poke needs from the world, injected so it can be driven without one.
 *
 * Same shape and same reason as {@link import('./channel-liveness.js').ChannelLivenessWorld}:
 * every member is a function of the outside world at the moment it is called, so
 * `verify-guardian-poke.mjs` drives **the shipped decision procedure** rather
 ...
 * that stays right.
 *
 * **There is deliberately no composer in here** — see the module header. A poke
 * on the composer is a Ctrl+C every thirty minutes, forever, by design.
 */
export interface GuardianWorld {
  /** The kill switch, read fresh — the same reader every other caller uses. */
  emissionEnabled: () => boolean;
  /** The guardian, read fresh from disk on every poke. */
  readConfig: () => GuardianConfig;
  /** Write the frame over the channel. NEVER a composer send — see above. */
  send: (address: AgentAddress, content: string) => GuardianRoute;
  now: () => number;
...
export function composePokeMessage(opts: {
  ...
  return (
    `${DAEMON_SENDER_TAG} guardian sweep poke #${opts.pokeNumber} — you are Butchr's guardian ` +
...
export function runGuardianPoke(opts: {
  ...
  const done = (
    outcome: GuardianPokeOutcome,
    detail: string,
    extra: Partial<GuardianPokeResult> = {}
  ): GuardianPokeResult => {
    const result: GuardianPokeResult = {
      outcome,
  ...
  if (!world.emissionEnabled()) {
    return done(
      'channel-disabled',
      'channel emission is switched off fleet-wide, so there is no carrier for a poke and ' +
  ...
  if (!config.address) {
    return done(
      'no-guardian',
      'no guardian is configured, so nothing was poked. This is not an error — it is the ' +
  ...
  if (!route.routed) {
    ...
    return done(
      'undelivered',
      `the poke was refused for ${describeAddress(address)} (${route.reason ?? 'no reason given'}): ` +
        ...
        `nobody chose. The next one is ${Math.round(intervalMs / 60000)} minutes away`,
      { address, reason: route.reason ?? 'unknown' }
    );
  }

  return done(
    'delivered',
    `a poke was written to ${describeAddress(address)}'s channel connection ` +
      ...
      `sweep was right are three further questions and none of them is answered here`,
    { address, connectionId: route.connectionId ?? null }
  );
}

/** The fleet-level state, as the board page and `list_agents` read it. */
export interface GuardianState {
  /** The configured guardian, or null when there is none. */
  address: AgentAddress | null;
  /** `false` means nobody is watching the fleet on a timer. */
  configured: boolean;
  intervalMs: number;
  setBy: string | null;
  setAt: string | null;
  /** The most recent poke of any kind. */
  lastPoke: GuardianPokeResult | null;
  /** The most recent poke that reached a connection. `null` = never, this daemon. */
  lastDelivered: GuardianPokeResult | null;
  /** How long ago that was, or null if it has never happened. */
  sinceLastDeliveryMs: number | null;
  /** Pokes since the last delivered one. */
  consecutiveUndelivered: number;
  /**
   * No poke has landed in {@link OVERDUE_INTERVALS} intervals.
   *
   * **This is the field that makes AC3 and AC5 true**: `Guardian: epic/KAN-203`
   * and `Guardian: epic/KAN-203 — last poke landed 4 hours ago` must not render
   * the same, and a renderer cannot make that distinction from an identity.
   */
  overdue: boolean;
  /** Every poke this daemon has made. */
  pokes: number;
  /** The last few, compacted — `list_agents` is polled every two seconds. */
  recent: { outcome: GuardianPokeOutcome; startedAt: string; reason: string | null }[];
  /**
   * WHAT THIS RECORD IS EVIDENCE OF, carried as data rather than as a comment.
   *
   * The literal `'delivery'`, and it is a literal type so that it cannot drift
   * to `'supervision'` under an author who is being helpful. See the module
   * header: a comment in this file is invisible to the board page, and the board
   * page is exactly where the overclaim would be made.
   */
  proves: 'delivery';
  /** The limit, in a sentence every surface renders verbatim. */
  provesDetail: string;
  /** One sentence for a reader who reads nothing else. */
  detail: string;
}
...
export class GuardianRecord {
  private readonly pokeList: GuardianPokeResult[] = [];
  private lastDeliveredResult: GuardianPokeResult | null = null;
  ...
  public record(result: GuardianPokeResult): void {
    this.total += 1;
    this.pokeList.push(result);
    if (this.pokeList.length > RECENT_LIMIT) this.pokeList.shift();
    ...
    if (result.outcome === 'undelivered') this.undelivered += 1;
  }
  ...
  private describe(
    ...
    if (!config.address) {
      return (
        'NO GUARDIAN IS SET, so nothing is watching this fleet on a timer. That is the default ' +
    ...
    if (this.total === 0) {
      return (
        `${who} is the guardian and is poked every ${mins} minutes. No poke has been sent yet on ` +
    ...
    if (overdue) {
      return (
        `NO POKE HAS REACHED ${who.toUpperCase()} IN ${this.undelivered} ATTEMPT(S)` +
    ...
    if (this.lastDeliveredResult) {
      return (
        `a poke last reached ${who} at ${this.lastDeliveredResult.startedAt}` +
        (sinceLastDeliveryMs !== null
    ...
    return (
      `${this.total} poke(s) to ${who}, none of which has been delivered yet. ` +
...
/** What the scheduler needs beyond one poke's world. */
export interface GuardianPokerOptions {
  world: GuardianWorld;
  record?: GuardianRecord;
  /** Overrides the configured interval. For a proof, not for production. */
  intervalMs?: number;
  firstPokeDelayMs?: number;
}
...
export class GuardianPoker {
  ...
  private readonly rec: GuardianRecord;

  public constructor(private readonly opts: GuardianPokerOptions) {
    this.rec =
      opts.record ??
  ...
  public start(): void {
    ...
    this.opts.world.log(
      `[Guardian] the supervision poke is on: ` +
        (config.address
   ...
   */
  public pokeOnce(): GuardianPokeResult {
    if (this.running) {
      return {
        ...
        elapsedMs: 0,
        detail:
          'a poke is already in flight; this request was not started rather than run alongside ' +
