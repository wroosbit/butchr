...
export interface RepoWorkState {
  /** Relative to the workspace: `.` for the workspace itself, else the subdir. */
  path: string;
  branch: string | null;
  ...
  /**
   * Commits on this branch that its upstream does not have. Null means the
   * question could not be answered — no upstream is configured — which is
   * itself worth showing: a branch that has never been pushed has *everything*
   * unpushed.
   */
  unpushedCommits: number | null;
  /** True when there is no upstream branch to compare against. */
  noUpstream: boolean;
}
...
export interface WorkState {
  workDir: string;
  /**
   * Whether the answer below is an answer. False means the check could not be
   * performed, and the caller must warn rather than reassure.
   */
  checked: boolean;
  repos: RepoWorkState[];
  /** True only when `checked` and something would actually be lost. */
  hasUnsavedWork: boolean;
  /** One sentence for a human, whichever of the above is the case. */
  summary: string;
}
 ...
 */
function findRepos(workDir: string, depth = 0): string[] {
  const found: string[] = [];
  if (fs.existsSync(path.join(workDir, '.git'))) found.push(workDir);
...
function readRepo(repo: string, workDir: string): RepoWorkState | null {
...
function repoHasWork(repo: RepoWorkState): boolean {
  if (repo.modifiedFiles > 0 || repo.untrackedFiles > 0) return true;
...
function describe(repo: RepoWorkState): string {
  const parts: string[] = [];
  if (repo.modifiedFiles > 0) parts.push(`${repo.modifiedFiles} changed`);
  if (repo.untrackedFiles > 0) parts.push(`${repo.untrackedFiles} untracked`);
  if (repo.noUpstream) {
 ...
 */
export function readWorkState(workDir: string): WorkState {
  const unchecked = (summary: string): WorkState => ({
    workDir,
    checked: false,
    repos: [],
  ...
  if (!workDir) return unchecked('No workspace directory is recorded for this agent, so its uncommitted work could not be checked.');
  ...
  if (repos.length === 0) {
    return unchecked(
      `No git repository was found in ${workDir}, so nothing can be said about uncommitted work. ` +
      'Anything the agent has not pushed elsewhere will still be lost.'
    );
  }

  const states = repos.map((repo) => readRepo(repo, workDir)).filter((s): s is RepoWorkState => s !== null);
  if (states.length === 0) {
    return unchecked(
      `git could not be run in ${workDir}, so this agent's uncommitted work could not be checked.`
    );
  }
