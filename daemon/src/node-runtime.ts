...
export const MIN_NODE_VERSION = { major: 20, minor: 18, patch: 1 } as const;

/** `20.18.1`, for messages. */
export const MIN_NODE_VERSION_TEXT =
  `${MIN_NODE_VERSION.major}.${MIN_NODE_VERSION.minor}.${MIN_NODE_VERSION.patch}`;

/** Where the requirement above came from, quoted in refusals and log lines. */
export const MIN_NODE_VERSION_SOURCE =
  'mcp-remote declares no engines of its own; its direct dependency undici@7 ' +
...
/** One directory that was considered, and what became of it. */
export interface RuntimeCandidate {
  /** The directory itself — a PATH entry, or the daemon's own bin directory. */
  dir: string;
  /** How this directory came to be considered, for the log line. */
  origin: 'daemon' | 'path';
  /** The Node it holds, if it holds one. */
  node?: string;
  /** The npx beside that Node, if there is one. */
  npx?: string;
  /** What `node --version` said, when it could be asked. */
  version?: string;
  /** Set when this candidate was not used, saying why in one clause. */
  rejected?: string;
}

export interface NpxRuntime {
  /** True when `node`, `npx` and `pathPrefix` below are all populated. */
  ok: boolean;
  /** Absolute path to the npx to invoke. */
  npx?: string;
  /** Absolute path to the Node that will run it and its children. */
  node?: string;
  /** That Node's version, as it reported it. */
  version?: string;
  /**
   * Directories to place ahead of PATH in the server process's environment —
   * exactly one entry, the directory the pair was found in. This is the part
   * that actually decides the interpreter; see the measurements above.
   */
  pathPrefix?: string[];
  /** Every directory considered, in the order it was considered. */
  candidates: RuntimeCandidate[];
  /**
   * The first `npx` on the search path, whether or not it was chosen.
   *
   * Recorded rather than recomputed at log time so that "we did not take the
   * obvious one" is a fact on the result and not only a sentence in a log line.
   * Resolution short-circuits on its first usable candidate, so when the
   * daemon's own directory wins the PATH entries are never examined at all and
   * `candidates` cannot answer this question.
   */
  firstNpxOnPath?: string;
  /** Set when `ok` is false: one sentence naming what is wrong and what to do. */
  problem?: string;
}

/** `v20.20.2` / `20.20.2` → {20,20,2}; anything else → null. */
export function parseNodeVersion(reported: string): NodeVersion | null {
...
export function meetsMinimum(version: NodeVersion, min: NodeVersion = MIN_NODE_VERSION): boolean {
 ...
 */
function reportedVersion(nodeBinary: string): string | null {
  try {
 ...
 */
function candidateDirs(searchPath: string): Array<{ dir: string; origin: 'daemon' | 'path' }> {
  const dirs: Array<{ dir: string; origin: 'daemon' | 'path' }> = [
    { dir: path.dirname(process.execPath), origin: 'daemon' }
  ];
  for (const entry of searchPath.split(':')) {
    if (!entry) continue;
    if (dirs.some((d) => d.dir === entry)) continue;
 ...
 */
export function resolveNpxRuntimeUncached(searchPath = process.env.PATH || ''): NpxRuntime {
  const candidates: RuntimeCandidate[] = [];
  const firstOnPath = firstNpxOnPath(searchPath);
  const found = (rest: Omit<NpxRuntime, 'candidates' | 'firstNpxOnPath'>): NpxRuntime => ({
    ...rest,
    candidates,
  ...
  for (const { dir, origin } of candidateDirs(searchPath)) {
    const candidate: RuntimeCandidate = { dir, origin };
  ...
  return found({
    ok: false,
    problem:
      `No Node on this machine is new enough to run mcp-remote: it needs at least ` +
...
const cache = new Map<string, NpxRuntime>();

/** Test seam. Production never calls this; the proof script does, per section. */
export function resetNpxRuntimeCache(): void {
  cache.clear();
}
 ...
 */
export function resolveNpxRuntime(searchPath = process.env.PATH || ''): NpxRuntime {
...
function firstNpxOnPath(searchPath: string): string | null {
...
function logResolution(resolved: NpxRuntime): void {
  ...
  console.log(
    `[NodeRuntime] mcp-remote will run on ${resolved.node} (${resolved.version}) via ${resolved.npx}; ` +
  ...
  if (first && first !== resolved.npx) {
    console.log(
      `[NodeRuntime] Note: this is NOT the first npx on PATH (${first}). ` +
      `It was preferred because ${
        resolved.candidates.find((c) => c.npx === resolved.npx)?.origin === 'daemon'
