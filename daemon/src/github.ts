...
export interface PullRequestSnapshot {
  ...
  isDraft: boolean;
  headRefName: string;
  /** The head commit. An approval does not survive a change to this. */
  headRefOid: string;
  mergedAt: string | null;
  /**
   * `APPROVED` | `CHANGES_REQUESTED` | `REVIEW_REQUIRED` | `''`.
   *
   * **Empty on every pull request this fleet has ever opened**, and that is
   * structural rather than an accident of timing: every agent authenticates as
   * one shared GitHub identity, so GitHub refuses a review verdict on a PR the
   * same account authored. An approval in this fleet arrives as a *comment*.
   * Anything reading this field must say "GitHub records no approving review"
   * and must not say "nobody approved" — see `pr-watch.ts`, `green-idle`.
   */
  reviewDecision: string;
  /** `CLEAN` | `BLOCKED` | `BEHIND` | `DIRTY` | `UNSTABLE` | `UNKNOWN` | … */
  mergeStateStatus: string;
  /**
   * The rollup of every check EXCEPT {@link APPROVAL_STATUS_CONTEXT}.
   *
   * Excluded because an unapproved pull request fails that status by design, and
   * folding it in here would make "the checks are green" unsatisfiable until
   * somebody approves — collapsing "this build is broken" into "nobody has
   * looked yet", which are the two things a notification most needs to keep
   * apart.
   */
  checks: CheckRollup;
  /** The names of checks that are not green, so a notice can say which. */
  failingChecks: string[];
  /** Whether KAN-306's approval status is recorded at this head. */
...
/**
 * One read of one repository — or the reason there wasn't one.
 *
 * The same discriminated shape as `JiraSnapshotOutcome`, deliberately: a failed
 * read must never be able to look like a successful read of an empty repository,
 * which is the shape in which "the watcher stopped working" hides. `backOff`
 * marks the failures that mean *ask less often* rather than *something is
 * broken*.
 */
export type GitHubOutcome =
  | { ok: true; prs: PullRequestSnapshot[] }
  | { ok: false; error: string; backOff: boolean };

/** What the watcher needs of GitHub. One method, so a proof can stub it. */
export interface GitHubReader {
  listPullRequests(repo: string): Promise<GitHubOutcome>;
}
...
export function repoFromRemoteUrl(url: string): string | null {
  ...
  const match =
    /^(?:https?:\/\/|ssh:\/\/)?(?:[^@/]+@)?github\.com[:/]+([^/\s]+)\/([^/\s]+?)(?:\.git)?\/?$/.exec(
      trimmed
    );
 ...
 */
export function originUrlFromGitConfig(text: string): string | null {
  let inOrigin = false;
 ...
 */
export function repoForCheckout(checkout: string): string | null {
  ...
  try {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (declared) commonDir = path.resolve(gitDir, declared);
      } catch {}
...
export function rollupOf(entries: any[] | null | undefined): {
  ...
  for (const entry of list) {
    ...
    if (typeof entry?.state === 'string') {
      ...
      else if (!green.has(state)) failing.push(name);
      continue;
    }
    ...
    if (!conclusion) pending = true;
    else if (!green.has(conclusion)) failing.push(name);
  }
...
export function snapshotFrom(repo: string, row: any): PullRequestSnapshot | null {
  ...
  const commentIds = (Array.isArray(row?.comments) ? row.comments : [])
    .map((comment: any) => (typeof comment?.id === 'string' ? comment.id : null))
  ...
  return {
    ...
    mergeStateStatus:
      typeof row?.mergeStateStatus === 'string' ? row.mergeStateStatus.toUpperCase() : 'UNKNOWN',
    checks,
    failingChecks,
    approval,
    commentIds
 ...
 * asking us to shed.
 */
export function isBackOff(text: string): boolean {
  return /rate limit|secondary rate|abuse detection|\b(?:429|502|503|504)\b|timed out|timeout|ETIMEDOUT|ECONNRESET|EAI_AGAIN|tls:|x509|connection refused|no such host/i.test(
    text
  );
}
 ...
 */
export function ghErrorText(raw: string): string {
  const redacted = raw
 ...
 */
export class GhCliGitHubReader implements GitHubReader {
  constructor(
    private readonly opts: {
      log: (...args: any[]) => void;
      /** Overridable so a proof does not need `gh` on the machine. */
      run?: (args: string[]) => Promise<{ ok: true; stdout: string } | { ok: false; text: string }>;
      timeoutMs?: number;
      limit?: number;
    }
  ) {}

  public async listPullRequests(repo: string): Promise<GitHubOutcome> {
    const args = [
      'pr',
      'list',
      '--repo',
    ...
    for (const row of parsed) {
      ...
      if (snapshot) prs.push(snapshot);
    }
    return { ok: true, prs };
  }

  private exec(args: string[]): Promise<{ ok: true; stdout: string } | { ok: false; text: string }> {
