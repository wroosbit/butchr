  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "READ `censusScope` BEFORE ANY COUNT YOU TAKE OFF THIS ANSWER (KAN-630): it names the POPULATION this answer covers — the host, the runtime and the workspaces root — and Butchr runs ONE DAEMON PER MACHINE with no link to any other, so this is a BOX ANSWER wearing a fleet answer's clothes. An agent staffed on another machine is absent from every field here and THAT ABSENCE IS NOT EVIDENCE THAT IT STOPPED. On 2026-08-21 three agents made that inference in one hour, from three different vantage points, and all three were wrong: the agents were working throughout and their PRs merged while one of the readings was being escalated to the guardian. Every instrument answered truthfully every time — the defect is in the completion, which is why the scope is a field rather than a caveat. An ABSENT `censusScope` means this daemon predates KAN-630 and its counts name no population at all. IT MATTERS PARTICULARLY HERE: `running: 0 task agents` is a TRUE sentence on a box that runs no task agents, and it is the sentence a supervisor reads as \"the fleet is idle\" before staffing over the top of work already in flight on another machine. Reports how many concurrent agents this machine can carry and how many more can be started right now. The cap is derived from the machine's own cores and memory, so it differs between machines; headroom additionally accounts for the CPU and memory actually in use right now, so a fleet that is compiling reports less room than the same fleet idle. The load average is reported (`load1`) but no longer gates anything — `cpuBusyCores`, measured from /proc/stat, is what the CPU term divides. Separately from those counts, a machine that is *stalled* — thrashing on swap or blocked on a failing disk — admits nothing at all however much CPU and memory are free: `stallPercent` is the share of the last 10s in which every non-idle task was stalled (/proc/pressure `full avg10`, the worse of io and memory), and `stalled` says whether it crossed `stallRefusePercent`. A `stallPercent` of null does not mean healthy — it means NO FIGURE WAS TAKEN and nothing is bounding I/O saturation, and `stallInstrument` says which of the two ways that happened (KAN-267): `absent` is a kernel with no /proc/pressure, which is a permanent property of that machine, and `unreadable` is a /proc/pressure that IS present and would not answer — a permission, a container namespace, a truncated read — which is a fault on that box worth going to look at. Neither refuses anything; `stallIoState`/`stallMemoryState` carry the same distinction per file, with a detail saying what happened. Ask this before activating, not after the machine is on its knees. ALSO REPORTS priorities: what each running agent is worth (epic 3, story 2, task 1), which is what an activation at capacity would have to strictly outrank before it could stand any of them down.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
      ...
      },
      {
        ...
        description:
          ...
          + "READ `censusScope` BEFORE ANY COUNT YOU TAKE OFF THIS ANSWER (KAN-630): it names the POPULATION this answer covers — the host, the runtime and the workspaces root — and Butchr runs ONE DAEMON PER MACHINE with no link to any other, so this is a BOX ANSWER wearing a fleet answer's clothes. An agent staffed on another machine is absent from every field here and THAT ABSENCE IS NOT EVIDENCE THAT IT STOPPED. On 2026-08-21 three agents made that inference in one hour, from three different vantage points, and all three were wrong: the agents were working throughout and their PRs merged while one of the readings was being escalated to the guardian. Every instrument answered truthfully every time — the defect is in the completion, which is why the scope is a field rather than a caveat. An ABSENT `censusScope` means this daemon predates KAN-630 and its counts name no population at all. Lists every running agent, from herdr's view of what exists rather than the daemon's session map — so agents that outlived a daemon restart are still listed. Each entry carries sessionless: true when the daemon is not attached to it, in which case the session-only fields (sessionId, url, createdAt, status) are null. Panes named like agents but with no agent behind them are reported separately under unbackedPanes and are not counted as agents. ALSO CHECK censusUnreadableRecordsTotal BEFORE YOU TRUST THE AGENT COUNT: it is how many registry rows the census could not read and therefore did NOT count, and a zero there is what makes the list above trustworthy. A non-zero means THIS LIST IS SHORT BY THAT MANY and the agents it omits are named in censusUnreadableRecords with the reason each was skipped — so an agent may be running with no row here, and concluding 'nothing is working on that' from a short list is exactly the mistake this field exists to prevent. `null` is NOT zero and must not be read as one: it means no disclosure reached this daemon at all — the census could not be taken, or the peer predates read-path contract v4 — so the count may be short with nothing able to say so. READ EACH ROW'S `standing` RATHER THAN WATCHING THE COUNT: the count never falls on its own, so a steady non-zero is the ordinary state and is NOT a fault, while ANY INCREASE is a real event. `standing.available: false` means the peer is too old to have rendered a verdict (`because: 'peer-below-v7'`) — that is a fact about the CONNECTION and is NOT the same as `standing: 'unknown'`, which is a verdict about the row; do not read either as an all-clear. Where `standing.available` is true, read the verdict at `standing.verdict` with its evidence `standing.claimsEvent` beside it: `'retired'` means the row records an agent being switched off and nothing was going to be restored from it, while `'claims-an-agent'` is the one to look at. For those, READ `supersession` RATHER THAN GUESSING: `matched` means a later readable row already covers that agent and there is nothing to do, `ran-found-nothing` means nothing readable supersedes it and IS the case worth acting on, and `could-not-run` means the row named no path to join on so THE QUESTION WAS NOT ANSWERED — that is not evidence either way, and treating it as 'lost' manufactures an alarm that never clears. Use `identity` to find the line in the registry by hand; never join on it, because it is in the row's own vocabulary and matches nothing in a path-keyed list. `standing.claimsAt` is a quotation of what the row said and is NOT guaranteed to parse as a date. All three v7 fields sit together behind `standing.available` because they arrive together: a peer below v7 sends none of them, and reading a missing `claimsAt` as 'the row named no timestamp' is the same collapse one field over. ALSO CHECK missingAgents: agents the durable registry records as active that herdr has no agent by THAT NAME for — a ticket of theirs may still read In Progress while nothing is working on it. READ EACH ROW'S `occupiedBy` AND `reason` BEFORE ACTING, and do NOT treat a non-empty missingAgents as a list of stopped work: the check that produces these rows compares NAMES, and an agent restarted under a name this daemon did not derive (CrabCast spells a pane `crabcast-kan-373-<hash>` where Butchr spells it `butchr-task-kan-373`) is absent from `agents` by construction while being perfectly alive. Since KAN-579 the row says which of three it is, and only one of them is a loss. `occupiedBy` non-null NAMES what the census reports live in that agent's own workDir under another name — that agent is very probably RUNNING, and standing it down or re-activating it would act on work in flight, so go and look at that directory rather than acting on the row. `occupiedBy: null` WITH a reason ending 'It is not running.' is the real loss: the directory was searched and nothing was in it, and that is the row to re-activate or stand down. `occupiedBy: null` with a reason saying the question COULD NOT BE ESTABLISHED is neither — the census was unreachable, or short by rows it could not read, so nothing was searched and the row is evidence of nothing; confirm the directory is idle yourself before touching it. An ABSENT `occupiedBy` key means this daemon predates KAN-579 and cannot answer, which is different from every value it could carry — treat its rows as the third case. Re-activating an agent RESUMES A CONVERSATION, which is why the distinction is worth the read. ALSO CHECK preemptedAgents: agents deliberately stood down to free capacity for higher-priority work, listed until they are put back. Their work was interrupted rather than finished, so their tickets must NOT be left In Progress — move each back to To Do with a comment naming what took its slot. Re-activating one resumes the conversation it was stopped in. standbyAgents is NOT a problem to fix: agents somebody switched off on purpose whose workspace is still on disk, listed so they can be started again (butchr_activate_agent with their type and key, and their recorded defaultAgent so they come back as what they were). standbyTotal is the unclipped count when more exist than are listed. ALSO CHECK each agent's `channel`: what its startup channel self-check found, and the carrier its messages will actually take. `transport: 'composer'` means that agent's channel loop did not prove out at bring-up, so a message to it will interrupt whatever it is doing — an agent silently on the composer while you believe it is on channels is exactly what this field exists to prevent. `outcome: 'unverified-client'` means the loop works but the Claude Code version it is running is one nobody has measured channel delivery on; channels are a research preview and the contract can move, so treat delivery to that agent as unproven and say so if you rely on it. `outcome: 'unchecked'` is NOT a fault: nobody has checked, usually because the agent outlived a daemon restart. READ `transport` RATHER THAN `outcome` FOR THE CARRIER — they are different questions and an unchecked agent can be on any of the three. `transport: 'unregistered'` means the agent holds no channel registration while the registry expects it to: a steer to it is REFUSED rather than delivered, it is the ordinary state for the first seconds after a daemon restart or a socket error, and it clears by itself when the agent's MCP server re-announces. Before KAN-274 this field read `'channel'` in that state and a send to such an agent interrupted it, so an older daemon's row cannot be trusted on this point. `clientVersion` is the client's own report of itself and is what pins any of this to a version. An absent `channel` field means this daemon cannot answer, which is different from every value it could carry. ALSO CHECK undeliveredNotifications: news the DAEMON ITSELF could not deliver — a ticket status change, a new comment, an agent of yours going blocked. Since KAN-301 the daemon never types a notification into a terminal, so a notification whose recipient held no channel registration is HELD rather than delivered by interrupt, and this is where it is visible. `pending` is recoverable: each entry names the recipient, how many notices are waiting, how long the oldest has waited and why the channel would not take it, and every one is retried on the daemon's 30-second sweep and delivered the moment that agent's registration comes back. `abandoned` is the one to act on: those notices waited past their window and were DROPPED, so the agents named there were never told and their Jira tickets are the only remaining record — go and read the ticket named in `lastSubject` rather than assuming somebody was informed. The abandoned count is never reset while this daemon lives. An empty `pending` and an empty `abandoned` together mean every notification this daemon produced reached a live channel. An ABSENT `undeliveredNotifications` field means this daemon does not track that, which is different from tracking it and holding nothing. ALSO CHECK boardControl.health: whether the board reconciler could establish, on its last cycle, that anybody MEANT to stop the agents it is not being told to run. Since KAN-342 an agent is stood down only where the board SAID something — a status that excludes its ticket, or an assignee that is somebody else's — and every other absence spares it. That evidence comes from one diagnostic query, so `health.diagnostic.answered: false` means NO AGENT CAN BE STOOD DOWN AT ALL while it lasts: a ticket moved to Done keeps its agent running, the fleet stops shrinking, and the capacity gate starts refusing real work while the reconciler goes on reporting that it converged. READ `consecutiveFailures` RATHER THAN `answered`, because they answer different questions: 1 is the ordinary one-off 5xx or timeout that the next cycle fixes and is NOT worth acting on, while a number in the tens means stand-downs have been off for that many minutes and somebody has to look — `failingSince` says when it started and `detail` carries Jira's own words, so a partial page is distinguishable from an outage. `health.agents` lists every agent left running for want of evidence, each with the `condition` that spared it: `undetermined` is the diagnostic being silent (the case above), while `no-assignee` is one ticket whose assignee field is empty and is fixed by assigning it to this machine's Jira account. ALSO READ `health.unstaffable`, which is a DIFFERENT POPULATION FROM `health.agents` AND IS THE ONE NOBODY IS RUNNING: EVERY OPEN ticket with an EMPTY assignee, so the reconciler's query can never return it. An agent for one of these is not stopped, it is NEVER STARTED, and no other field mentions it — a ticket in this state looks exactly like a ticket nobody has got to yet. Fix one by assigning it to this machine's Jira account. READ `health.unstaffable.askedJql` BEFORE `answered` AND BEFORE ANY COUNT YOU TAKE OFF `tickets`, BECAUSE AN ABSENT AUDIT AND A CLEAN BOARD ARE OTHERWISE THE SAME BYTES (KAN-649): it is the JQL this daemon actually sent, scope and all, and a build that carries NO cross-door query cannot produce it. The field `unstaffable` is OLDER THAN THE QUERY BY A WHOLE TICKET, so its presence establishes nothing and its empty result is available in two worlds at once — 'the query ran over every door and the board is clean' and 'this build has no cross-door query, so nothing was asked'. ⚠ NO `askedJql` MEANS THE SECOND, and an empty `tickets`, a bare `[]` or a `null` from such a daemon IS NOT A CLEAN BOARD — read it as NOBODY ASKED and go and check what build is running. This was measured: on 2026-08-21 the running daemon answered this field `[]` from a build whose source did not contain the query at all — its dist predated the query's merge and it had not been restarted since — and nothing in the response said so. WITH `askedJql` present the report is about every door, and comparing it against the query you expected is how you check the scope as well. THEN READ `health.unstaffable.answered`, BECAUSE A FAILED READ AND A CLEAN BOARD ARE DIFFERENT SHAPES HERE AND NOT AN EMPTY LIST versus a null (KAN-597): `answered: true` carries `tickets`, where `[]` genuinely means the query ran and the board is clean; `answered: false` carries NO ticket list at all and means NOBODY LOOKED, with `consecutiveFailures`, `failingSince` and Jira's own `detail` saying for how long and why. There is deliberately no value on the failing branch that can be mistaken for a clean board. ⚠ SINCE KAN-597 IT COVERS EVERY OPEN STATUS, To Do INCLUDED, AND IT IS FED BY A QUERY OF ITS OWN rather than by the diagnostic — an older daemon reports the In Progress / In Review subset only, as a bare array or null, and its silence about To Do is not a claim that To Do is clean. ⚠ AND AN OLDER DAEMON'S SILENCE IS EXACTLY WHAT `askedJql` MAKES VISIBLE: a bare array, a null, or an object with no `askedJql` are all the same answer — this response is not evidence about any door. WHY THE WIDER POPULATION IS THE ONE WORTH READING: the proxy's atlassian_create_issue assigns every ticket IT files (KAN-577), but the official Atlassian MCP server and the web UI reach the same site without passing that guard, and a ticket filed through one of those is born in To Do with no assignee — three were, in the ninety minutes after the guard deployed. This field is the only thing on this response that sees them. `health: null` is NOT a healthy board — it means no cycle has completed yet, which is the ordinary state for the first minute after a daemon restart. An ABSENT `boardControl` field means no reconciler is wired here at all. ALSO READ `prWatch.strandedMerges`, WHICH IS THE ONLY FIELD ON THIS RESPONSE THAT REPORTS AN APPROVED PULL REQUEST NOBODY HAS MERGED (KAN-260). `prompts/task.md` permits exactly one party to press the merge button — the task agent whose ticket the branch names — so a pull request that has been approved and not merged is stuck on ONE agent, and NOTHING ELSE HERE SAYS SO: the board reads In Review, which is true and reads as progressing; `gh pr list` reads CLEAN; and the agent, if it is on standby, reads as deliberately switched off. Four sat like that at once on 2026-08-10 and were found hours later by a supervisor reading this census for an unrelated reason. EACH ROW NAMES `issueKey`, `hold` AND `liveMergers`, AND `liveMergers` IS THE FIELD TO READ RATHER THAN A FILTER TO APPLY: EMPTY means no live agent on this box holds that ticket, so nothing here can land it until one is started (butchr_activate_agent), while NON-EMPTY means an agent that CAN merge has not — which is NOT a milder case and is the one that disproved this ticket's original diagnosis, because an approval arrives as a GitHub comment and an agent that stopped to wait for it may never have been told. `hold` says what it is waiting on: `nobody-has-pressed-it` is one `gh pr merge` away, `the-head-does-not-merge` needs `gh pr update-branch` or a conflict resolved FIRST and the approval will not survive that, and `a-check-is-not-green` is not waiting on the approval at all. `waitingMs` is null when the approval predates this watcher's memory of it — THAT IS 'HOW LONG IS UNKNOWN', NEVER 'APPROVED JUST NOW'. ⚠ READ `answered` BEFORE `pulls`, AND NOTE THE TWO BRANCHES ARE DIFFERENT SHAPES RATHER THAN ONE SHAPE WITH A FLAG: `answered: false` carries NO `pulls` KEY AT ALL — it carries `because`, naming which repository reads failed — so there is deliberately no empty list on the failing branch that can be read as a clean fleet. ⚠ AND ON `answered: true`, READ `scannedRepos` BEFORE YOU TREAT AN EMPTY `pulls` AS GOOD NEWS: an EMPTY `scannedRepos` means this tick asked about NO repository, which is the ordinary state when no live agent holds a checkout and nothing is outstanding in memory — a complete answer about an empty scope, not a clean fleet. ⚠ AN ABSENT `strandedMerges` KEY MEANS THIS DAEMON PREDATES KAN-260 AND NOBODY ASKED — read it exactly as KAN-649's absent `askedJql`, and go and check what build is running rather than concluding that nothing is stranded. AND EVERYTHING HERE IS A READING OF THIS BOX (KAN-630): an agent holding the ticket on another machine is absent from `liveMergers` by construction.",
        inputSchema: {
          type: "object",
          properties: {
            view: {
