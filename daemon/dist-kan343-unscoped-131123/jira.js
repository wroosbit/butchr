...
class JiraRequestError extends Error {
    status;
    legs;
    constructor(message, status, legs = []) {
        super(message);
        this.status = status;
        this.legs = legs;
...
export function extractDetail(raw, parsed) {
    const candidate = (Array.isArray(parsed?.errorMessages) && typeof parsed.errorMessages[0] === 'string'
 ...
 */
export class TokenJiraTransport {
    cred;
    gatewayOrigin;
    cloudId = null;
    cloudIdLeg = null;
     ...
     */
    constructor(cred, gatewayOrigin = 'https://api.atlassian.com') {
        this.cred = cred;
    ...
    async attempt(leg, base, method, path, 
    ...
    async get(path, signal, product = 'jira') {
     ...
     */
    async write(method, path, body, signal, product = 'jira') {
        return this.request(method, path, body, signal, product);
    }
    async request(method, path, body, signal, product = 'jira') {
        const legs = [];
        const cloudId = await this.resolveCloudId(signal);
        if (this.cloudIdLeg)
            legs.push(this.cloudIdLeg);
        ...
        if (viaSite.status === undefined) {
            throw new JiraRequestError(this.scrub(`no leg completed: ${legs.map(describeLeg).join('; ')}`), undefined, legs);
        }
 ...
 */
export function failureKind(err, signal) {
...
function traceOf(res) {
    const id = res.headers.get('atl-traceid') ??
 ...
 */
export function explainLegs(legs) {
    ...
    if (decisive?.failure === 'timeout') {
        return say(cloud?.failure === 'timeout'
            ...
            : `Atlassian did not respond within ${VALIDATE_TIMEOUT_MS}ms. Your credential was neither accepted nor rejected — this is a timeout, not a refusal.`, 'timeout');
    }
    if (decisive?.failure === 'network') {
        return say(`Could not reach ${hostOf(decisive.endpoint)}. Check the site address and your connection. Nothing about your token was tested.`, 'network');
    }
    ...
    if (cloud && (cloud.failure || (cloud.status !== undefined && cloud.status !== 200))) {
        ...
        return say(`${hostOf(cloud.endpoint)} answered, but not as a Jira site (HTTP ${cloud.status} from its cloud-ID endpoint). Check the site address. This request carries no credential, so your email and token were not involved.`, 'site-not-jira');
    }
    if (cloud && cloud.status === 200 && cloud.detail === 'response contained no cloudId') {
        return say(`${hostOf(cloud.endpoint)} responded but did not identify itself as a Jira site. Check the site address; your credential was not involved.`, 'site-not-jira');
    }
    // Cases 2/3: the gateway had an opinion.
    if (gateway?.status === 403) {
        return say('The token authenticated at the Atlassian gateway but is not permitted to read Jira (403). It needs the read:jira-work scope.', 'gateway-forbidden');
    }
    ...
    if (site?.status === 401) {
        if (gateway?.status === 401) {
            ...
            return say('Both endpoints rejected this email and token (401). The gateway at api.atlassian.com refuses tokens that are missing the scope it needs; your site host refuses tokens that are wrong, expired, or revoked. Since neither accepted it, the most likely causes are a revoked token, a token pasted with a character missing, or an email that is not the one that owns the token.', 'credentials-rejected');
        }
        return say(`Reached ${hostOf(site.endpoint)}, but it rejected this email and token (401). The address is right and the site is up — so the token is wrong, expired, or revoked, or the email is not the account that owns it.`, 'credentials-rejected');
    }
    if (site?.status === 403) {
        return say(`Reached ${hostOf(site.endpoint)} and the credential authenticated, but the account is not permitted to read Jira (403).`, 'site-forbidden');
    }
    if (gateway?.status === 401 && site?.status !== undefined && site.status !== 200) {
        return say(`The Atlassian gateway rejected the token (401) and ${hostOf(site.endpoint)} returned HTTP ${site.status}.`, 'gateway-token-rejected');
    }
    if (site?.status === 404) {
        return say(`Reached ${hostOf(site.endpoint)}, but there is no Jira REST API there (404). Check the site address.`, 'site-not-jira');
    }
...
export class JiraClient {
     ...
     */
    async getIssueSnapshot(key, signal) {
        const { status, body } = await this.transport.get(`/rest/api/3/issue/${encodeURIComponent(key)}?fields=${WATCH_FIELDS}`, signal);
        if (status !== 200) {
     ...
     */
    async searchBoard(jql, maxResults, signal) {
        const { status, body } = await this.transport.get(`${SEARCH_PATH}?jql=${encodeURIComponent(jql)}` +
            `&fields=${BOARD_FIELDS}&maxResults=${maxResults}`, signal);
        if (status !== 200) {
     ...
     */
    async getIssueTypeName(key, signal) {
        const { status, body } = await this.transport.get(`/rest/api/3/issue/${encodeURIComponent(key)}?fields=issuetype`, signal);
        if (status !== 200) {
     ...
     */
    async proxyRead(path, signal, product = 'jira') {
     ...
     */
    async proxyWrite(method, path, body, signal, product = 'jira') {
     ...
     */
    async credentialStillWorks(signal) {
        try {
    ...
    async validate(signal) {
        ...
        if (work.status === 200) {
            return {
                valid: true,
                legs: work.legs,
                note: 'The token can read Jira work, which is all Butchr needs. It cannot read your account profile, so there is no name to show — add the read:jira-user scope if you want one.'
            };
        }
 ...
 */
export function snapshotFrom(key, body) {
    const fields = body?.fields ?? {};
    const commentIds = Array.isArray(fields?.comment?.comments)
        ? fields.comment.comments
            .map((comment) => (comment?.id === undefined ? '' : String(comment.id)))
 ...
 */
export function boardPageFrom(body, maxResults) {
    ...
    for (const row of rows) {
        ...
        issues.push({
            ...
            assigneeAccountId: typeof assigneeAccountId === 'string' && assigneeAccountId ? assigneeAccountId : null,
            assigneeDisplayName: typeof assigneeDisplayName === 'string' && assigneeDisplayName ? assigneeDisplayName : null
        });
    }
    const complete = body?.isLast === true ||
...
export function explainProxyFailure(status, legs, write = false, 
...
sent) {
    ...
    if (status === 403 && write) {
        ...
        return {
            error: "The Butchr daemon's own Atlassian credential was refused for a WRITE (HTTP 403). It " +
    ...
    if (status === 401 || status === 403) {
        ...
        const explained = explainLegs(legs.filter((leg) => leg.leg !== 'cloud-id'));
        return {
            error: `The Butchr daemon's own Atlassian credential was refused (HTTP ${status}). This is ` +
    ...
    if (status === 404) {
        return {
            error: `${said_by} answered 404 for this request. The daemon's credential worked; the issue, ` +
    ...
    if (status === 429) {
        return {
            error: 'Atlassian is rate-limiting this credential (429). It is shared by the whole fleet, ' +
...
function failedValidation(err) {
    const legs = Array.isArray(err?.legs) ? err.legs : [];
    if (legs.length)
        return { valid: false, legs, ...explainLegs(legs) };
    return {
        valid: false,
        diagnosis: 'network',
...
export class JiraIssueTypeService {
     ...
     */
    constructor(store, timeoutMs = LOOKUP_TIMEOUT_MS, now = () => Date.now(), makeTransport = (cred) => new TokenJiraTransport(cred), 
    ...
    boardTimeoutMs = BOARD_TIMEOUT_MS) {
        this.store = store;
        ...
        this.boardTimeoutMs = boardTimeoutMs;
    }
    status() {
     ...
     */
    async setCredential(cred) {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), this.validateTimeoutMs);
        try {
            const result = await new JiraClient(this.makeTransport(cred)).validate(controller.signal);
            if (!result.valid)
                return result;
     ...
     */
    async getIssueTypeName(key) {
        const cached = this.cache.get(key);
        if (cached !== undefined)
            return cached;
        // Recently failed: do not pay the timeout again yet.
        if (this.now() < this.failingUntil)
            return null;
    ...
    async lookup(key) {
        ...
        try {
            ...
            console.warn(`jira: issue-type lookup for ${key} failed (${err?.message ?? 'unknown error'}); ` +
        ...
        try {
            ...
            if (!page.complete) {
                return {
                    ok: false,
                    backOff: false,
                    error: `the board search returned a partial page (${page.issues.length} issue(s), ` +
     ...
     * Never throws, like everything else this service exposes.
     */
    async proxyRead(path, product = 'jira') {
     ...
     * Never throws, like everything else this service exposes.
     */
    async proxyWrite(method, path, body, product = 'jira') {
        return this.proxyCall(method, path, body, product);
    }
    async proxyCall(method, path, body, product = 'jira') {
        ...
        if (!transport) {
            ...
            return {
                ok: false,
                credentialFault: true,
                error: `The Butchr daemon has no Atlassian credential configured, so it cannot make this ` +
        ...
        try {
            ...
            const { status, body: responseBody, legs } = write
                ? await client.proxyWrite(method, path, body, controller.signal, product)
            ...
            const refusedLeg = legs.some((leg) => leg.leg !== 'cloud-id' && (leg.status === 401 || leg.status === 403));
            if (!explained.credentialFault && refusedLeg) {
                ...
                if (verdict === 'refused') {
                    return {
                        ...
                        legs,
                        credentialFault: true,
                        error: `Atlassian answered ${status} for this ${write ? 'write' : 'read'}, but that is ` +
                ...
                if (verdict === 'unknown') {
                    return {
                        ...
                        legs,
                        credentialFault: false,
                        error: `${explained.error} (Butchr could not confirm whether its own credential is still ` +
            ...
            return {
                ...
                ...(legs.length ? { legs } : {}),
                ...(explained?.diagnosis ? { diagnosis: explained.diagnosis } : {}),
                error: `The Butchr daemon could not reach Atlassian at all for this ${write ? 'write' : 'read'}.` +
                    (write
