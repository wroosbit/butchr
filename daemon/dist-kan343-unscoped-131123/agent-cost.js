 ...
 */
export function classifyTree(pids, readArgv = readCmdline) {
    for (const pid of pids) {
        ...
        if (t < 0)
            continue;
        const type = argv[t + 1];
        if (typeof type !== 'string' || type === '' || type.startsWith('--'))
            continue;
 ...
 */
export function subtreeTicks(procs, pids) {
    let ticks = 0;
    for (const pid of pids) {
        const p = procs.get(pid);
        if (p)
            ticks += p.cpuTicks + p.childCpuTicks;
    }
    return ticks;
}
/**
 ...
 */
export function totalsOf(trees) {
    return {
        agents: trees.length,
 ...
 */
export function finishMeasurement(start, isSupervisor, readArgv = readCmdline, activityOf = () => 'unknown') {
 ...
 */
export function aggregateTrees(before, after, elapsed, isSupervisor, readArgv = readCmdline, activityOf = () => 'unknown') {
    ...
    for (const [root, pids] of groups) {
        const ticks = Math.max(0, subtreeTicks(after, pids) - subtreeTicks(before, groupsBefore.get(root) ?? []));
        let rss = 0;
        for (const pid of pids) {
        ...
        agents.push({
            ...
            activity: marker.workspaceType === null
                ? 'unknown'
    ...
    const supervisorTrees = marked.filter((a) => isSupervisor(a.workspaceType));
    const chargeableTrees = marked.filter((a) => !isSupervisor(a.workspaceType));
 ...
 */
export async function measureAgentCost(seconds, isSupervisor, activityOf = () => 'unknown') {
