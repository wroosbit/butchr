...
import { coreMcpServerDefinitions, prepareWorkspaceMcpServers, unusableMcpServers } from './launchers.js';
...
// there.
import { AmbiguousKeyError, addressFromAgentName, agentNameFor, ambiguousKeyMessage, typeFromAgentName, workspaceDirFor } from './herdr.js';
import { sharedStartLedger } from './start-ledger.js';
...
import { beginBuildCoercions, operationByTool, proxyReport, refuseProxyCall, refuseWriteOutsideCaller, selectedProxyMode, takeBuildCoercions } from './atlassian-proxy.js';
...
import { capacityReason, capacityRefusal, describeCapacity, effectiveCeilingOf, readCapacity, summarizeCapacity } from './capacity.js';
/**
 ...
 */
function refuseUnusableMcpServers(defs) {
    const unusable = unusableMcpServers(defs);
    if (unusable.length === 0)
        return null;
    return (`MCP server${unusable.length > 1 ? 's' : ''} that cannot start on this machine: ` +
...
export class MessageRouter {
     ...
     */
    standDownVerdict(result, closedType, key) {
        if (result.success)
        ...
        if (!closedType) {
            return {
                ...
                reason: 'no workspace type could be resolved for this key, so no census row could be ' +
        ...
        if (!census.reachable) {
            return {
                outcome: 'unverifiable',
                reason: 'the agent runtime did not answer, so nothing could be established about whether ' +
        ...
        if (record)
            return { outcome: 'still-running', record };
    ...
    nudgeIfResumed(session, defaultAgent) {
        if (!session.resume || !needsResumeNudge(session.resumedConversation))
            return;
    ...
    unintendedStart(type, key, agentName, reattachOnly) {
        if (reattachOnly && !this.hasLiveAgent(agentName)) {
            return {
                ...
                error: `Nothing to re-attach to: ${this.herdrBridge.runtimeName} has no live agent ` +
                    `named ${agentName}. ` +
    ...
    async handleActivate(data, respond) {
        ...
        if (!session) {
            ...
            session = this.herdrBridge.spawnSession(config.type, key, data.url, renderedPrompt, config.priority, isSupervisorType(config.type), data.defaultAgent, prepareWorkspaceMcpServers(mcpServers, { type: config.type, key }), resume, 
    ...
    async handleActivateByKey(data, respond) {
        ...
        if (!session) {
            ...
            session = this.herdrBridge.spawnSession(type, key, url, renderedPrompt, priority, isSupervisorType(type), defaultAgent, prepareWorkspaceMcpServers(mcpServers, { type, key }), resume, 
            ...
            ...(verdict.outcome === 'still-running'
                ? {
                    stillRunning: {
                        ...
                        detail: 'The stand-down was RECORDED but the agent was NOT stopped: this daemon holds ' +
    ...
    handleSendToAgent(data, respond) {
        ...
        (result) => {
            ...
            const claims = sealComposerClaims({
                ...
                enteredTranscriptBasis: typed && pane.submitted === false
            ...
            respond({
                ...
                error: typed && pane.submitted === false
                ...
                composerLeftHolding: typed && pane.submitted === false
                    ? {
                        text: tagged,
                        priorContentCleared: pane.interrupted === true,
                        advice: `The message was typed onto ${address.type}/${address.key}'s composer and NOT ` +
