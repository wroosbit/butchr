 ...
 * not an estimate.
 */
export const MEASURED_AGENT_COST = {
 ...
 */
export const COST_SOURCE_BY_PROVENANCE = {
    measured: 'measured',
    restored: 'restored',
    stale: 'stale'
};
...
export function costSourceOf(measured) {
 ...
 */
export function boundCoresByObservedCpu(cores, agentTrees, busyCores) {
    if (!Number.isFinite(cores) || cores <= 0)
        return null;
    if (!Number.isFinite(agentTrees) || agentTrees <= 0)
        return null;
    if (!Number.isFinite(busyCores) || busyCores <= 0)
        return null;
    const impliedFleetCores = cores * agentTrees;
    if (impliedFleetCores <= busyCores)
        return null;
    const used = busyCores / agentTrees;
    if (!Number.isFinite(used) || used <= 0 || used >= cores)
        return null;
 ...
 */
export function startingAgentCost(cost) {
    return {
 ...
 */
export function unobservedStartsAmong(startedAt, measured, now) {
    ...
    if (!measured)
        return { count: inFlight.length, because: 'no-measurement' };
 ...
 */
export function worstStall(stall) {
    if (!stall)
        return null;
    const candidates = [];
 ...
 */
export function humanReserveCores(cores) {
 ...
 */
export function humanReserveBytes(totalBytes) {
 ...
 */
export function computeCapacity(machine, running, options = {}) {
    ...
    const cost = { residentBytes: resident.value, cores: coreCost.value };
    ...
    const capByMemory = Math.floor(Math.max(0, machine.totalBytes - reservedBytes - supervisorReserve.bytes) /
        cost.residentBytes);
    let cap;
    let capBoundBy;
    ...
    const cpuBusySource = typeof machine.busyCores === 'number' ? 'measured' : 'load-average';
    const cpuBusyCores = cpuBusySource === 'measured'
        ? Math.max(0, Math.min(machine.cores, machine.busyCores))
        : Math.max(0, Math.min(machine.cores, machine.load1));
    const cpuBusyWindowSeconds = cpuBusySource === 'measured' ? machine.busyWindowSeconds ?? null : null;
    ...
    const liveCoresBound = coreCost.source === 'override' || cpuBusySource !== 'measured'
        ? null
    ...
    const liveCpuBudget = machine.cores - cpuBusyCores - reservedCores - unobservedStarts.cores;
    const headroomByCpu = Math.max(0, Math.floor(liveCpuBudget / liveCoreCost));
    const headroomByMemory = Math.max(0, Math.floor(Math.max(0, machine.availableBytes - reservedBytes - unobservedStarts.bytes) /
        cost.residentBytes));
    ...
    const countingBoundBy = headroomByCap <= headroomByCpu && headroomByCap <= headroomByMemory
        ? 'cap'
    ...
    const headroomBoundBy = stalled && headroomBeforeStall > 0 ? 'stall' : countingBoundBy;
    return {
        machine,
        cost,
        costSource,
 ...
 */
export function readAvailableBytes() {
    try {
 ...
 */
export function readPressureFull(path) {
    try {
        const line = fs
 ...
 */
export function readStallFacts(root = '/proc/pressure') {
    return {
...
function readCpuTicks() {
    try {
        ...
        if (v.length < 5 || v.some((n) => !Number.isFinite(n)))
            return null;
        const idle = v[3] + v[4];
...
export function readMachineFacts() {
...
function envNumber(name, allowZero = false) {
    const raw = process.env[name];
    if (raw === undefined || raw.trim() === '')
        return undefined;
    const value = Number(raw);
    if (!Number.isFinite(value) || (allowZero ? value < 0 : value <= 0)) {
        console.warn(`${name}=${raw} is not a ${allowZero ? 'non-negative' : 'positive'} number; ignoring it`);
        return undefined;
    }
    return value;
}
...
export function optionsFromEnv() {
    ...
    if (memoryMb !== undefined)
        overrides.residentBytes = memoryMb * MIB;
    if (cores !== undefined)
    ...
    return {
        ...
        supervisorMemoryOverride: supervisorMemoryMb !== undefined ? supervisorMemoryMb * MIB : null
    };
}
/**
 ...
 */
let liveMeasuredCost = null;
export function setMeasuredAgentCost(measured) {
    liveMeasuredCost = measured;
}
export function getMeasuredAgentCost() {
 ...
 */
export function readCapacity(running, supervisors = 0, 
/**
 ...
 * is about.
 */
export function effectiveCeilingOf(c) {
    const ceiling = c.running + c.headroom;
    const stability = c.headroomBoundBy === 'cap'
        ? 'static'
    ...
    const arithmetic = c.headroomBoundBy === 'cap'
...
export function describeCapacity(c) {
    const m = c.machine;
    const lines = [];
    lines.push(`machine: ${m.cores} cores, ${gib(m.totalBytes)} RAM ` +
    ...
    lines.push(c.cpuBusySource === 'measured'
    ...
    lines.push(`agent cost: ${Math.round(c.cost.residentBytes / MIB)} MB resident (${c.costSource.residentBytes}), ` +
    ...
    if (c.liveCoresBound) {
        ...
        lines.push(`  contradicted: ${b.published} core × ${b.agentTrees} agent tree(s) = ` +
    ...
    if (c.unobservedStarts.count > 0) {
        ...
        lines.push(`starts in flight: ${u.count} agent(s) admitted that no instrument has priced — ${why}. ` +
    ...
    if (c.measured) {
        ...
        lines.push(`  ${restored ? 'restored (damped)' : stale ? 'stale (damped)' : 'measured (damped)'}: ` +
        ...
        lines.push('  no live measurement; seed figures are the 2026-07-31 constants, ' +
            'not a measurement of this fleet');
    }
    lines.push(`reserved for you: ${c.reservedForHuman.cores} core(s), ${gib(c.reservedForHuman.bytes)}`);
    if (c.capBoundBy === 'configured') {
        lines.push(`cap: ${c.cap} task agents (set by BUTCHR_MAX_AGENTS, derivation skipped)`);
    }
    else {
        lines.push(`cap: ${c.cap} task agents — ` +
    ...
    if (c.supervisorReserve.count > 0) {
        lines.push(`supervisor memory reserve: ${gib(c.supervisorReserve.bytes)} ` +
    ...
    lines.push(`running: ${c.running} task agent(s)` +
        (c.supervisors > 0
    ...
    if (c.stallPercent === null) {
        lines.push('io/memory stall: no /proc/pressure on this machine (needs Linux 4.20+ with CONFIG_PSI), ' +
        ...
        const both = `${typeof io === 'number' ? `${io.toFixed(2)}% io` : 'io unreadable'}, ` +
            `${typeof mem === 'number' ? `${mem.toFixed(2)}% memory` : 'memory unreadable'}`;
        lines.push(`io/memory stall: ${both} (/proc/pressure \`full avg10\` — the share of the last 10s in ` +
    ...
    lines.push(`headroom: ${c.headroom} more — ` +
    ...
    lines.push(`effective ceiling: ${ceiling.ceiling} task agent(s) alive — working OR ` +
        ...
        ` (this figure ${ceiling.stability === 'projects'
            ? 'holds as starts land — they are charged to the term that set it'
            : ceiling.stability === 'static'
                ? 'is fixed until the cap is changed'
                : 'describes this moment only — the term that set it measures activity, not population'}). It is a reading and not a constant: quote it with the time it was taken. ` +
 ...
 */
export function summarizeCapacity(c) {
    ...
    const provenanceNote = c.costSource.cores === 'seed'
 ...
 */
function unobservedClause(c) {
    const u = c.unobservedStarts;
    if (u.count < 1)
        return '';
    return (`; ${u.cores.toFixed(2)} core and ${gib(u.bytes)} of that is ${u.count} agent(s) ` +
...
export function capacityReason(c) {
    if (c.headroomBoundBy === 'stall') {
        ...
        const what = c.stallSource === 'memory'
            ? 'stalled reclaiming memory — swap thrash, not a shortage of free memory'
            : 'stalled on I/O';
        return (`this machine spent ${c.stallPercent?.toFixed(2)}% of the last 10 seconds with every ` +
    ...
    if (c.headroomBoundBy === 'cpu') {
        ...
        return (`${c.cpuBusyCores.toFixed(2)} of this machine's ${c.machine.cores} cores are already ` +
    ...
    if (c.headroomBoundBy === 'memory') {
        return (`only ${gib(c.machine.availableBytes)} of memory is available, ` +
            `${gib(c.reservedForHuman.bytes)} of that is held back for you` +
            unobservedClause(c));
    }
    return (`${c.running} task agent${c.running === 1 ? ' is' : 's are'} already running ` +
 ...
 */
export function capacityHeadline(c) {
    const constraint = c.headroomBoundBy === 'cpu'
        ? 'not enough cpu'
        : c.headroomBoundBy === 'memory'
            ? 'not enough memory'
...
export function capacityRefusal(c, what) {
    ...
    const remedy = c.headroomBoundBy === 'stall'
