 ...
 */
export function jiraEventNudgeText(events, relation) {
    const list = Array.isArray(events) ? events : [events];
    const key = list[0].key;
    const clause = (event) => event.kind === 'status'
    ...
    return (`${DAEMON_SENDER_TAG} ${key} ${what} ${whose[relation]} ` +
...
export class JiraPollState {
    ...
    issues = new Map();
    loaded = false;
    constructor(file = POLL_STATE_PATH, now = () => Date.now()) {
        this.file = file;
        this.now = now;
    }
    /**
    ...
    load() {
        ...
        try {
            ...
            console.error(`[jira-poll] ${this.file} is not readable JSON (${e?.message ?? String(e)}); ` +
                `starting from empty state. Watched issues will be re-initialised without notifying.`);
            return;
        }
        for (const [key, value] of Object.entries(parsed?.issues ?? {})) {
            ...
            this.issues.set(key.toUpperCase(), {
                ...
                linkedKeys: Array.isArray(value.linkedKeys)
                    ? value.linkedKeys.filter((linked) => typeof linked === 'string')
                    : []
            });
        }
    }
    ...
    save() {
        ...
        const body = JSON.stringify({ version: 1, issues: Object.fromEntries(this.issues) }, null, 2);
 ...
 */
export function compareCommentIds(a, b) {
    const na = Number(a);
    const nb = Number(b);
...
export class JiraPoller {
    ...
    start() {
        ...
        if (!this.state.isLoaded())
            this.state.load();
        this.opts.log(`[jira-poll] watching live-agent issues every ${this.intervalMs / 1000}s ` +
     ...
     */
    async pollOnce() {
        const tick = { polled: [], events: [], nudges: [], skipped: [], degraded: this.degraded };
        if (!this.state.isLoaded())
            this.state.load();
        let agents;
        try {
        ...
        for (const key of pollable) {
            ...
            if (!outcome.ok) {
                ...
                this.opts.log(`[jira-poll] ${key}: read failed` +
        ...
        for (const { event, snapshot } of pending) {
            ...
            if (bucket)
                ...
                byIssue.set(key, { snapshot, events: [event] });
        }
    ...
    recognise(snapshot) {
        ...
        if (!seen) {
            ...
            this.opts.log(`[jira-poll] ${key}: first sight — recording status \`${snapshot.statusName ?? 'unknown'}\` ` +
        ...
        const fresh = snapshot.commentIds.filter((id) => compareCommentIds(id, seen.maxCommentId) > 0);
        if (fresh.length) {
    ...
    updatePace(sawBackOff, sawSuccess) {
        if (sawBackOff) {
            if (!this.degraded) {
                this.degraded = true;
                this.opts.log(`[jira-poll] Jira asked to be left alone (429/5xx or an unreachable host); ` +
        ...
        if (this.degraded && sawSuccess) {
            this.degraded = false;
            this.opts.log(`[jira-poll] Jira is answering again; back to ${this.intervalMs / 1000}s between polls.`);
        }
    }
    /**
    ...
    newsFor(event, agent) {
        ...
        if (!authorship || event.kind !== 'comment')
            return event;
     ...
     */
    eventsFor(events, agent, relation, tick) {
        const mine = [];
        for (const event of events) {
            ...
            if (relation === 'own' && event.kind === 'status')
                continue;
            ...
            if (!forThisAgent) {
                this.opts.log(`[jira-poll] ${event.key} (${event.kind}): not telling ${agent.type}/${agent.key} ` +
     ...
     */
    async notify(events, snapshot, byKey, running, tick) {
        ...
        for (const agent of own) {
            ...
            if (supervisorName === agent.agentName)
                continue; // Guarded at write time too.
            if (!running.has(supervisorName)) {
                log(`[jira-poll] ${issueKey} (${kinds}): supervisor ` +
        ...
        if (parentKey && parentKey !== issueKey.toUpperCase()) {
            ...
            if (!parents.length) {
                ...
                log(`[jira-poll] ${issueKey} (${kinds}): board parent ${parentKey} ` +
        ...
        for (const { agent, relation } of targets.values()) {
            ...
            if (!forThisAgent.length)
                continue;
            log(`[jira-poll] ${issueKey} (${forThisAgent.map((e) => e.kind).join('+')}): ` +
