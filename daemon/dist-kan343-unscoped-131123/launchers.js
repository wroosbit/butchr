 ...
 * stale.
 */
export function coreMcpServerDefinitions() {
    return {
        [CORE_MCP_SERVER]: {
 ...
 */
export function withWorkspaceIdentity(defs, identity) {
    const core = defs[CORE_MCP_SERVER];
    if (!core || !identity.type || !identity.key)
        return defs;
    return {
        ...defs,
        [CORE_MCP_SERVER]: {
 ...
 */
export function materializeMcpServers(defs) {
    const out = {};
 ...
 */
export function prepareWorkspaceMcpServers(defs, identity) {
    return materializeMcpServers(withWorkspaceIdentity(defs, identity));
}
/**
 ...
 */
export function unusableMcpServers(defs) {
    return Object.entries(defs)
        .filter(([, definition]) => typeof definition.unusable === 'string' && definition.unusable !== '')
        .map(([name, definition]) => [name, definition.unusable]);
}
...
export function writeWorkspaceMcpConfig(workDir, defs) {
...
export function configureClaudeSettings(workDir) {
...
export function trustClaudeWorkspace(workDir, configPath) {
    ...
    const read = () => {
        ...
        try {
            ...
            return {
                unreadable: `${claudeConfigPath} exists but is unparseable; refusing to overwrite it ` +
                    `(${e?.message ?? String(e)})`
            };
        }
    };
    const trusted = (config) => config.projects?.[trustKey]?.hasTrustDialogAccepted === true;
    for (let attempt = 1; attempt <= TRUST_WRITE_ATTEMPTS; attempt++) {
        ...
        if (trusted(current.config)) {
            if (attempt > 1) {
                console.log(`[Launchers] Trust entry for ${trustKey} stuck after ${attempt - 1} write attempt(s)`);
            }
        ...
        try {
            ...
            const error = `Failed to record workspace trust in ${claudeConfigPath}: ${e?.message ?? String(e)}`;
        ...
        console.error(`[Launchers] Trust entry for ${trustKey} was clobbered after write ` +
            `${attempt}/${TRUST_WRITE_ATTEMPTS} — a concurrent writer rewrote ${claudeConfigPath}`);
    }
    const error = `Trust entry for ${trustKey} in ${claudeConfigPath} would not stick after ` +
 ...
 */
export function developmentChannelArgv() {
 ...
 */
export function developmentChannelFlags() {
 ...
 */
function claudeCommand(promptCommand, channelFlags) {
 ...
 */
const RETIRED_LAUNCHERS = {
    'anti-gravity': "the 'anti-gravity' launcher was removed in KAN-395 (2026-08-14) — Butchr launches " +
 ...
 */
export function resolveLauncher(name) {
    ...
    if (!launcher) {
        throw new Error(`Unknown agent '${requested}'. Valid launchers: ${Object.keys(AGENT_LAUNCHERS).join(', ')}. ` +
