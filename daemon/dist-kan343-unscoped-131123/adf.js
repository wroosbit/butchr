...
const COERCIONS = {
    ...
    'listItem>blockquote': {
        to: 'unwrap',
        why: "Jira's ADF validator rejects a blockquote inside a list item (400 INVALID_INPUT), " +
 ...
 */
function conflictingMarks(target, marks) {
    ...
    for (const [owner, allowed] of Object.entries(companions)) {
        ...
        for (const other of present) {
            if (other === owner || allowed.includes(other))
                continue;
 ...
 */
function normaliseMarks(node, target) {
    const conflicts = conflictingMarks(target, node.marks);
    if (conflicts.length && node.marks) {
        const companions = MARK_COMPANIONS[target];
        const owners = Object.keys(companions).filter((owner) => node.marks.some((mark) => mark.type === owner));
        ...
        pendingCoercions.push(`${dropped.join(' and ')} dropped from a code span: Jira's ADF validator rejects a text ` +
 ...
 */
export function jiraAdfViolations(payload) {
    const found = [];
    const seen = new Set();
    ...
    const walk = (node, parent) => {
        ...
        if (type === 'text' && Array.isArray(node.marks)) {
            for (const combination of conflictingMarks('jira', node.marks)) {
                ...
                found.push(`a text node marked ${combination}${quoted}. Jira's ADF schema lets a code span carry ` +
...
const text = (value, marks) => marks && marks.length ? { type: 'text', text: value, marks } : { type: 'text', text: value };
 ...
 */
export function inlineToAdf(source) {
    const out = [];
    let buffer = '';
    const flush = (marks) => {
        if (buffer) {
    ...
    while (i < source.length) {
        ...
        for (const { pattern, mark, wordBoundary } of INLINE_MARKS) {
            ...
            // cost before it did.
            if (wordBoundary && i > 0 && WORD_CHAR.test(source[i - 1]))
                continue;
        ...
        if (BULLET.test(line.body) || ORDERED.test(line.body)) {
            ...
            while (i < lines.length) {
                ...
                if (!head.body) {
                    ...
                    if (!next || !next.body || next.indent < at)
                        break;
                    i += 1;
                    continue;
                }
                ...
                if (head.indent === at && !match)
                    break;
                if (!match && head.indent <= at)
                    break;
                ...
                const own = [{ raw: match ? (ordered ? match[2] : match[1]) : head.body, indent: 0, body: (match ? (ordered ? match[2] : match[1]) : head.body).trim() }];
                i += 1;
                while (i < lines.length) {
                    const cont = lines[i];
                    if (!cont.body) {
                        const next = lines[i + 1];
                        if (!next || next.indent <= at)
                            break;
...
const isDelimiterRow = (body) => /^\|?\s*:?-{1,}:?\s*(\|\s*:?-{1,}:?\s*)*\|?$/.test(body) && body.includes('-');
const splitRow = (body) => body
    .replace(/^\|/, '')
    .replace(/\|$/, '')
    .split('|')
    .map((cell) => cell.trim());
 ...
 */
let pendingCoercions = [];
let currentTarget = 'confluence';
function contain(parent, blocks) {
    ...
    return blocks.flatMap((block) => {
        ...
        if (!coercion) {
            throw new AdfConversionError(`A ${block.type} cannot go inside a ${parent} in ADF, and this converter has no ` +
        ...
        if (coercion.to === 'paragraph') {
            ...
            return [
                paragraph((block.content ?? []).map((node) => ({
                    ...node,
        ...
        return [
            {
                type: 'blockquote',
                content: (block.content ?? []).map((row) => paragraph([text((row.content ?? []).map(plainText).join(' | '))]))
            }
        ];
    });
}
...
const tokensOf = (value) => value.match(WORDS) ?? [];
/**
 ...
 */
function contentTokens(markdown) {
 ...
 */
export function markdownToAdf(markdown, target = 'confluence') {
    ...
    try {
        ...
        if (missing.length) {
            throw new AdfConversionError(`The markdown→ADF conversion would have lost ${missing.length} token(s) from your ` +
 ...
 */
export function confluenceBody(doc) {
...
export function adfToText(doc) {
    ...
    const renderInline = (nodes) => (nodes ?? []).map((n) => renderNode(n, '')).join('');
    ...
    const text = renderNode(doc, '').replace(/\n{3,}/g, '\n\n').trim();
