...
import { createAgentRuntime } from './runtime-switch.js';
import { MessageRouter } from './router.js';
import { JiraIssueTypeService } from './jira.js';
import { CredentialStore } from './credentials.js';
import { LD_API_ORIGIN, LaunchDarklyIntegration, createLaunchDarklyIntegration } from './integrations/launchdarkly.js';
...
import { AgentConnectionRegistry, addressFromAnnouncement, describeAddress } from './agent-connections.js';
import { buildFromAnnouncement } from './mcp-build.js';
import { CHANNEL_META_KEY_PATTERN, CHANNEL_SWITCH_PATH, carrierFor, channelEmissionEnabled, routeChannelMessage, writeChannelSwitch } from './channel.js';
import { freshConnectionFrom, superviseChannelStartup } from './channel-startup.js';
import { CHANNEL_SELFCHECK_ACTION, CHANNEL_SELFCHECK_RESULT_ACTION, ChannelSelfCheckAckRegistry, ChannelSelfCheckStore, runChannelSelfCheck } from './channel-selfcheck.js';
import { ChannelLivenessProbe } from './channel-liveness.js';
import { GuardianPoker, readGuardianConfig, writeGuardianConfig, setGuardian, clearGuardian } from './guardian.js';
...
import { BoardReconciler } from './board-reconcile.js';
import { boardControlReport } from './board-control.js';
import { CommentAuthorship } from './comment-authorship.js';
import { startMeasurement, finishMeasurement } from './agent-cost.js';
import { dampCost, sampleFromMeasurement, supervisorMemoryFromMeasurement } from './agent-cost-damping.js';
import { COST_ESTIMATE_PATH, clearCostEstimate, loadCostEstimate, saveCostEstimate } from './agent-cost-store.js';
import { MEASURED_AGENT_COST, getMeasuredAgentCost, sampleCpuBusy, setMeasuredAgentCost } from './capacity.js';
import { decideOnMissingSample, staleMeasurementMaxAgeMs } from './cost-sampler-policy.js';
...
if (daemonLog.repair.repaired) {
    log(`[log-repair] daemon.log carried ${daemonLog.repair.bytes} non-text byte(s) in ` +
...
if (!fdUsage) {
    ...
    log(`herdr fd limit: soft ${fdUsage.softLimit}, ${fdUsage.openFds} open, ` +
...
function ldApiOrigin() {
    ...
    try {
        ...
        console.log(`launchdarkly: BUTCHR_LAUNCHDARKLY_API_ORIGIN="${raw}" is not a URL — ignoring it and using ` +
            `${LD_API_ORIGIN}.`);
        return LD_API_ORIGIN;
    }
    ...
    if (!loopback) {
        console.log(`launchdarkly: BUTCHR_LAUNCHDARKLY_API_ORIGIN="${raw}" is not a loopback address — REFUSED, ` +
...
for (const integration of registry.integrations()) {
    ...
    log(`Integration ${integration.id} (${integration.name}): ` +
        (integration.enabled
...
log(`MCP servers assembled for a spawning agent now: ` +
...
log(credentialStatus.configured
...
for (const line of formatStalenessReport(getStalenessReport({ repoRoot, daemonStartedAt, force: true }))) {
    log(line);
}
/**
...
setTimeout(() => {
    ...
    if (!item)
        return;
    log(`deploy reach: restarting this daemon did NOT restart any agent's MCP server — ${item.headline}`);
    log(`        ${item.detail}`);
    if (item.remedy)
        log(`        fix: ${item.remedy}`);
 ...
 */
let managedMemo = null;
function expectedAgentNames() {
    let key;
    try {
    ...
    if (key && managedMemo?.key === key)
        return managedMemo.names;
    const names = new Set();
    ...
    if (key)
        managedMemo = { key, names };
    return names;
}
function isManagedAgent(address) {
...
const channelLiveness = new ChannelLivenessProbe({
    world: {
        ...
        candidates: () => agentConnections.addresses().flatMap((address) => {
            ...
            if (channelSelfChecks.degraded(address, conn.id))
                return [];
        ...
        readPane: async (address) => (await herdrBridge.tailAgent(address.key, address.type, 200)).text ?? null,
 ...
 */
const handleConnectionAction = (socket, msg) => {
    const reply = (body) => writeJsonLine(socket, msg?.id !== undefined ? { ...body, id: msg.id } : body);
    switch (msg?.action) {
        case 'hello': {
            ...
            log(`Connection ${connection.id} is ${describeAddress(connection.address)}` +
                ...
                // in one line.
                (connection.build
                    ? `, pid ${connection.build.pid} up since ${connection.build.startedAt}, loaded ${connection.build.loadedBuildAt ?? 'an unreadable build'}`
        ...
        case 'channel_send': {
            ...
            log(`channel_send → ${describeAddress(address)}: ` +
            ...
            reply({
                ...
                ...(outcome.droppedMetaKeys
                    ? {
                        ...
                        warning: `these meta key(s) will NOT reach the recipient: ${outcome.droppedMetaKeys.join(', ')}. ` +
                    ...
                    : {}),
                claim: `the frame was written to ${outcome.connectionId}; this is not a claim that the agent read it, ` +
        ...
        case 'channel_liveness': {
            ...
            if (msg?.run === true) {
                ...
                const clamp = (v, lo, hi) => typeof v === 'number' && Number.isFinite(v)
                ...
                if (started)
                    void channelLiveness.runOnce(overrides).catch(() => { });
                reply({
    ...
    void superviseChannelStartup({
        ...
        world: {
            ...
            pressEnter: (confirmation) => {
                log(`[ChannelStartup] ${describeAddress(address)}: pressing Enter — the live dialog is ` +
...
herdrBridge.setSessionEndedListener((event) => {
    log(`Session ended: ${event.sessionId} (${event.type}/${event.key}) ` +
...
const server = net.createServer((socket) => {
    ...
    const router = new MessageRouter(registry, promptLoader, herdrBridge, (msg) => writeJsonLine(socket, msg), broadcast, {
        jira,
        install: {
            repoRoot,
            daemonStartedAt,
        ...
        channelRoute: (address, content, meta) => routeChannelMessage({
            registry: agentConnections,
            reach: channelReach,
            address,
            content,
    ...
    onJsonLines(socket, (msg) => {
        try {
            if (handleConnectionAction(socket, msg))
                return;
            router.handle(msg);
        }
        catch (err) {
 ...
 */
const daemonRouter = new MessageRouter(registry, promptLoader, herdrBridge, () => { }, broadcast, {
    jira,
    install: {
        repoRoot,
        daemonStartedAt,
...
const notificationRoute = (address, content) => routeChannelMessage({
    registry: agentConnections,
    reach: channelReach,
    address,
    content,
...
const guardianPoker = new GuardianPoker({
    ...
    ...(process.env.BUTCHR_GUARDIAN_FIRST_POKE_MS
        ? {
            firstPokeDelayMs: Math.min(10 * 60_000, Math.max(1_000, Number(process.env.BUTCHR_GUARDIAN_FIRST_POKE_MS) || 0))
        }
        : {}),
    world: {
 ...
 * started.
 */
function boardReconcileMode() {
    const raw = (process.env.BUTCHR_BOARD_RECONCILE ?? '').trim().toLowerCase();
    if (raw === 'converge' || raw === 'report' || raw === 'off')
        return raw;
    if (raw) {
        log(`[board] BUTCHR_BOARD_RECONCILE is set to "${raw}", which is not one of ` +
 ...
 */
function reportBoardControl(agents) {
...
const boardReconciler = new BoardReconciler({
    ...
    runningAgents: () => daemonRouter.surveyFleet().agents.map((agent) => ({
        agentName: agent.agentName,
        type: agent.type,
    ...
    deactivate: async (agent) => {
        let response = null;
        daemonRouter.handleDeactivateByKey({ type: agent.type ?? undefined, key: agent.key }, (msg) => {
            response = msg;
        });
        return {
            success: response?.success === true,
...
function sweepForMissingAgents() {
    ...
    for (const name of announcedMissing) {
        if (!names.has(name))
            announcedMissing.delete(name);
    }
    for (const agent of missing) {
        if (announcedMissing.has(agent.agentName))
            continue;
        announcedMissing.add(agent.agentName);
        log(`AGENT LOST: ${agent.agentName} (${agent.type}/${agent.key}) is recorded as active ` +
...
let costSamplerState = 'no-measurement';
/**
 ...
 */
function degradeCostMeasurement(reason) {
    costEstimate = null;
 ...
 */
function noSampleThisWindow(gap) {
    ...
    if (costSamplerState !== 'stale') {
        ...
        log(`Agent-cost sampler: ${decision.reason}; holding the last measurement — ` +
...
function restoreCostEstimate() {
    ...
    log(`Agent-cost sampler: resumed from the estimate the previous daemon left in ` +
...
function sampleFleetCost() {
    ...
    if (!measurement)
        return; // first tick only opens the window
    const sample = sampleFromMeasurement(measurement, os.totalmem());
    if (!sample) {
        noSampleThisWindow(measurement.chargeable.agents <= 0
            ? // Named separately from an empty machine because it is now a state a
                ...
                // inventing one out of supervisors are different acts (KAN-365).
                {
                    ...
                    reason: measurement.totals.agents > 0
    ...
    if (costSamplerState !== 'live') {
        ...
        log(`Agent-cost sampler: live measurement established — damped cost ` +
...
function onListen() {
    ...
    void reconcileAgents({
        ...
        const idle = restored.filter((o) => needsResumeNudge(o.resumedConversation) && o.nudged === false);
        log(`[reconcile] Done: ${result.expected} expected, ` +
        ...
        const unregistered = survivors.filter((o) => agentConnections.resolve({ type: o.type, key: o.key }) === undefined);
        if (unregistered.length) {
            log(`[reconcile] ${unregistered.length} of ${survivors.length} surviving agent(s) hold no ` +
            ...
            log(`[reconcile] All ${survivors.length} surviving agent(s) hold a channel registration.`);
        }
